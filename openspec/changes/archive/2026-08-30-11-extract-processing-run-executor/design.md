...
6. Retain the existing post-fallback logger-only no-city conditional as a source-compatible guard; because every `HasMatch` result has a non-null city after `WithFallbackCity`, it has no reachable Skipped disposition. Preserve the reachable no-admin-match branch as skipped-store insert followed by Skipped and its existing warning.
