...
- Add a new terminal outcome, exit value, production lock key, default backend, or broader production refactor. The narrowly compatible evidence-classifier alignment is limited to recognizing the existing Failed exits 3 (busy), 4 (domain), and 5 (infrastructure).
...
The integration apphost invokes the same accepted-session executor/reporter entry and real lock collaborator used by production. It replaces only the collaborator invoked after successful lock admission for domain execution through the existing narrow internal `IProcessingRunDomainOperation.ExecuteAsync` seam, which receives the guarded session, the exact private production-domain closure, and the linked cancellation token. Production registration continues to invoke that exact closure; the seam exposes neither a lease nor count context, public flag, default-backend switch, or test-mode branch.

The real lock path queries and verifies ownership before it invokes the controlled operation. The operation invokes the exact supplied production-domain closure once against the empty isolated database, thereby producing the required real zero-eligibility event before any typed log. It then atomically publishes its entered marker, backend PID/application marker, and normal typed log handshake, proving the real gate admitted the owner even though the no-work query has already run; the operation remains held until an explicit release, cancel, or connection-loss signal. A successful release returns without invoking the closure again. Failure throws the controlled ordinary executor exception so block 23 selects domain failure/4; cancellation and detected ownership loss flow through the linked token to the existing Cancelled/130 and infrastructure Failed/5 paths. No marker means the domain operation was never reached. A busy contender never enters the operation, determines eligibility, or emits a hold marker.
...
When database creation is unavailable, require the configured database name to begin with `immich_reversegeo_test_` as an explicit dedicated-integration safety marker, and require the configured role to own that database. Before admission, under the test-harness serialization lease, verify that its `public` schema is empty and that the exact key is free; an unknown owner leaves the database untouched. Cleanup may release the lease after a bounded completed attempt, but every later admission revalidates ownership, schema emptiness, and key freedom. All cases still have unique run IDs, process roots, marker paths, and application names. The key remains `-7970420658158250032`; using random keys would avoid the contract under test.
...
Use the applied real coordinator/bridge/finalizer composition around the launched session wherever prior APIs allow. The direct external owner and coordinator-launched contender remain separate processes; local admission is therefore not mistaken for database contention. For each observable run, assert the expected terminal receipt, exact counters, one summary/fatal effect as defined by block 30, no active activity, callbacks closed, no supplementary contradiction, no retry, disposal complete, and only the matching coordinator generation released. Existing classifier compatibility recognizes only Failed exits 3, 4, and 5; it prevents a spurious terminal-exit mismatch for a healthy-output infrastructure-loss row while preserving strict anomalies for any unrelated exit or evidence defect.

Receipt origin remains the provenance boundary during final anomaly collection. A WorkerTerminal receipt permits terminal-derived exit/result pairing and post-terminal protocol/projection checks. A ControlPlane receipt proves the exact committed UI winner but does not retroactively turn missing-terminal crash evidence into worker testimony; the finalizer keeps its initial control-plane authority and failure category. Independent late input/output transport, kill, cleanup, and shutdown anomalies remain reportable for either receipt origin.
...
### 11. Complete controller input only after an accepted terminal

The production-descriptor smoke establishes a transport-specific liveness defect: the parent has accepted, flushed, and projected an exact correlated terminal, yet the child can remain alive while `Console.OpenStandardInput` is blocked in a native read. Cancellation/disposal of that local read alone is not a portable guarantee that it will unblock. The normal terminal path therefore needs the paired transport action: peer EOF from the controller once terminal authority is established.

Immediately after a fully validated terminal for the exact active session is recorded and before sink callback admission, the parent seals admission of new stdin writes and starts a separately tracked terminal-input completion action. The existing sole writer remains the only owner: a canonical write already admitted before that seal, including its flush, is allowed to settle; then the existing memoized controller-input close owner half-closes stdin exactly once. The stdout pump does not await that action, so a suppressed callback or callback failure cannot keep the input stream open. Before process exit, this action observes the actual stream `DisposeAsync` failure rather than suppressing it: it records the narrow `TerminalInputCloseFailed` containment reason/fact, preserves the accepted terminal, publishes the existing terminal-preventing observation safely, and enters the existing bounded containment lifecycle classified as InputTransport. It must settle its shared close attempt without self-joining; containment joins that task only from a different owner. Post-exit cleanup remains best effort. Invalid framing, a wrong correlation, an unaccepted terminal, or a terminal belonging to another session cannot cause this normal half-close.

The terminal-close failure is a separate monotonic session observation from the first terminal-preventing fault. If an earlier sink fault has already stopped callbacks—whether a receipt was stored or the later terminal callback is suppressed or throws—the finalizer consumes that first fault once and continues monitoring the terminal-close observation until exit or evidence finality. A later physical close failure therefore still reaches the existing same-session containment owner; it cannot be hidden by the earlier sink fault, replace the first containment cause, rewrite the accepted terminal or receipt, or create another timer, close attempt, or Stop request. Final classification continues to report InputTransport independently of the earlier failure.

This successful half-close is a transport-completion action, not Stop. Successful completion does not latch cancellation intent, send or fabricate a cancel, create or reset a grace deadline, request a tree kill, cancel the executor, add an input/control-token cancellation path, or change accepted-terminal/raw-exit authority. A physical pre-exit close failure instead joins the existing bounded containment policy, including its existing deadline and escalation behavior; it does not replace the accepted terminal, retry closure, fabricate exit, or add a parallel evidence owner. Stop and fault paths retain their existing tokens and deadline behavior. The parent still owns the process and waits for actual exit, both redirected-stream drains, finalizer settlement, and disposal before the matching coordinator handle becomes idle. The child retains its existing structured command-pump cancellation/disposal/join behavior, but after terminal it can complete a pending read through the accepted terminal's peer EOF. No detached pump, native-platform-I/O subsystem, public CLI or protocol extension, schema/key/default-backend switch, or CI change is introduced.

**Alternative considered:** depend only on local cancellation/disposal of `Console.OpenStandardInput`. Rejected because the pending native read is not reliably cancellable across the supported process transport. **Alternative considered:** make terminal input closure a Stop path. Rejected because it would invent cancellation/grace/kill semantics after an authoritative terminal and corrupt raw evidence.
...
- **[A test setting could target production]** → Require explicit test-only configuration, disposable/dedicated role ownership, safe naming, and documentation that refuses an unconfirmed target before worker launch.
- **[Fixed-key cases interfere in one shared database]** → Prefer database-per-case; otherwise require a dedicated role-owned empty database, serialized admission with key inspection, and separate databases for concurrent test processes.
...
- **[Terminal projection leaves a worker blocked on stdin]** → Half-close only the exact accepted terminal's controller input through the existing writer/close owners, then require actual exit, both drains, finalizer settlement, and disposal before idle.
...
Rollback removes only the block-32 integration apphost, test harness/tests, and maintainer provisioning guidance. It retains the narrow compatible production repairs: the internal post-lock domain-operation seam, the Failed 3/4/5 classifier compatibility and receipt-origin boundary, and the accepted-terminal input half-close with its typed pre-exit containment path. It changes no production schema, lock identity, public protocol or CLI, default backend, default test selection, or CI workflow.
