...
The invalid role/mode paths run before application logging and retain their existing constant-form stderr diagnostics. After successful public resolution and safe logger construction, emit 6601 once. Enforce exact role mappings: Standard/Web-only use Web, Run-once uses RunOnce, and only InternalWorker has null mode and no 6601; readiness kind follows that role. Stopping records the first owned shutdown reason immediately; stopped records the existing final process outcome after cleanup. In the absence of a later retained failure, completed maps to completed, host shutdown to cancelled, and startup/fatal failure to failed. A later retained cleanup or shutdown failure changes only the final 6605 outcome to failed/Warning, never replays 6604, and never changes exit precedence. Capture stop time at the original boundary rather than delaying 6604 until final outcome is known. InternalWorker precedence never reads public mode and therefore emits no 6601; its role lifecycle uses nullable `deployment_mode=null`. Neither templates nor fields contain raw arguments, the private selector token, environment variable values, or command descriptors. `application_role=internal-worker` is a bounded role value, not disclosure of invocation syntax.
...
Map the landed validator/classifier's typed failures into a finite bounded `violation_code`; never interpolate raw input, exception messages, raw/stored stderr, command descriptors, or protocol payloads. Emit 6630 at the first retained protocol violation for the job. Emit 6640 only after an accepted terminal receipt; it does not fabricate terminal observations for crash/missing-terminal paths. Emit exactly one 6641 after classifier-required process/stream/bridge finality. Its projection consumes the retained classifier category and anomalies. Normal agreeing terminal/exit tuples, mapped/unmapped/unavailable presence, readiness, forced-stop facts and PID continuity follow the specification; an observed conflicting terminal-plus-exit tuple becomes `terminal-exit-mismatch`. Agreeing exits do not erase later transport, protocol, projection or cleanup failures, and raw exit numbers do not establish managed authority without a terminal. Invalid combinations fail tests rather than being logged. A pre-ready crash therefore has no 6612/6640 but does have 6641 with `ready_observed=false`, null terminal, and the actual available/unavailable memory fields.
...
Consume block 65's exact bounded internal observation snapshot after coalescer finality. Emit 6650 only when `enqueue_wait_count>0` or `replaced_count>0`, and at most once for the job. Copy its accepted replaceable/lossless counts, replaced and delivered-snapshot counts, FIFO high-water, enqueue-wait count/duration, projection duration, stale rejection count, and abnormal abandonment count without inventing capacity or suppressed-sequence fields. Copy ordinary cadence dispatches from the exact primary-read-model owner observation, frozen by the existing cadence owner before release; retain existing lifetime counters unchanged. Add only `finality_kind=terminal|nonterminal`; copy terminal-flush duration only for terminal finality and use null for nonterminal finality. It does not instrument every replacement, producer wait, progress event, UI callback, or notification tick. No event is emitted for an unsaturated job.
...
## Apply-time prerequisite reconciliation

The landed cadence observation is cumulative across Bind calls. Event6650 must use an additive owner-scoped ordinary-dispatch count produced by the existing cadence owner, frozen before release, never a telemetry-owned subtraction of lifetime totals. ProcessingState/Lookup/cache controller are the respective primary read models; separate process-wide status and final notifications are excluded. This observational extension preserves existing lifetime counters, queue/cadence behavior and ownership.

The planned isolated exit matrix must not supersede the landed classifier. Retained startup/protocol/transport/projection/cleanup decisions take precedence; terminal authority and independent anomalies remain distinct. Event6641 follows those decisions and maps invalid no-terminal code3 to protocol-failed, and existing code4 execution failure after readiness to worker-failed. Agreeing-terminal tuples are the normal-case mapping, not permission to hide a later independent anomaly. No pure classifier or lifecycle policy is changed.
