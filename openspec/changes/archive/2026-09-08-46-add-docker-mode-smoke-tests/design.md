...
Add scripts/docker-mode-smoke.sh as the canonical implementation and test:docker-smoke as its stable local/CI entry. Bash is available on the supported Ubuntu runner and can use Docker CLI, curl, strace, and standard process/file tools without putting test utilities into the chiseled image. The existing Ubuntu job installs only the missing strace prerequisite. The script fails before Docker work unless passwordless privilege escalation and ptrace attach are usable, then uses strict shell options, run-unique names/labels, an explicit image tag, and helper functions for deadline polling, assertions, diagnostics, and cleanup.
...
Start one pinned PostgreSQL test image per harness run on a run-unique internal bridge network, with no host-published port and run-local credentials. Standard and Web-only also join a second run-labeled ordinary bridge used only for their loopback-published HTTP port; PostgreSQL and non-Web cases remain solely on the internal bridge. This gives the Web cases the egress capability of an ordinary bridge, so the no-download assertions remain mandatory. Wait with pg_isready plus a SQL query before applications start. A checked-in SQL fixture creates only columns/indexes required by the landed quoted asset/asset_exif queries and offers reset states:
...
For Standard, use exactly two bounded scheduled phases in one absent-mode container. The first admits one deterministic ocean asset and runs the child to completion, proving the bundled no-country result, persisted skip, terminal lifecycle, and no download. The second enables the fixture-only PostgreSQL hold after detector admission, proves the live child command and UID/GID, attaches host-side tracing to every observed parent/child thread, and records the live mapping before stopping the parent. Success requires a parent thread's kernel `wait4` return for the mapped namespace child PID before the parent leader exits; container namespace teardown alone is insufficient. These are complementary functional observations because a child cannot be both terminal and live; they are not a repeated-worker soak. For Web-only, use the identical saved schedule and cross enough bounded due windows to cover both Standard opportunities. Require positive disabled-policy text plus no role-scoped SQL, scheduler, pending, or child evidence. Use raw server-rendered curl responses only so an interactive Dashboard statistics request cannot contaminate the zero-SQL proof. Exact markers must come from landed bounded status/log contracts, not arbitrary exception text. A negative observation alone is insufficient.
...
Every asynchronous helper takes a named finite deadline. The private attach deadline is derived from readiness, each intervening observation, and EOF reap budgets, and its exact wait status distinguishes wrapper timeout from successful EOF. On any failure, capture image inspect/history, each container inspect/log/exit state, Docker process snapshots, port mappings, PostgreSQL logs/readiness and non-secret fixture state, mount trees/ownership, and an assertion manifest. Commands first capture under the removable raw root and atomically write redacted evidence. A final defense-in-depth scan marks any discovered secret as a failure and rewrites or removes that evidence before retention. Use traps for EXIT/INT/TERM; preregister intended container names, authorize removal only by the exact run label, reconcile all exact-label resources, then verify none remain. Terminate the Standard parent through Docker and require its traced `wait4` reap of the mapped active child before force-removal fallback; record fallback as a test failure, not success.
