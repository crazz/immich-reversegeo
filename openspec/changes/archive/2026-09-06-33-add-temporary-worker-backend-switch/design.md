...
The registration method validates the enum with an exhaustive switch and throws `ArgumentOutOfRangeException` for any undefined cast before building or starting the host. At dispatch, the coordinator defensively reuses that same exhaustive validation guard before keyed resolution. There is no string parser and no binding from `IConfiguration`, `AppConfig`, settings JSON, environment variables, command-line arguments, endpoints, or Razor components.
...
The in-process adapter forwards those exact arguments once to the block-11 singleton executor. The child adapter delegates the same request and run ID to the existing `WorkerRunControlPlane`; that owner creates one block-25 session, binds one block-27 bridge to the exact reporter/state adapter already armed by the coordinator, and lets block 30 classify/finalize complete session evidence. The adapter returns the `ProcessingRunResult` corresponding to the already-authoritative committed terminal; returning the value does not report a second terminal.

The coordinator retains its existing admitted order: freeze selection on the exact handle, publish the exact handle/request/CTS, mark pending, arm the exact reporter, resolve the one frozen key, and dispatch once. Rejected triggers do not resolve a backend. The coordinator never invents a backend-specific result, and backend completion cannot release a nonmatching handle.
...
The coordinator owns one CTS and one active/stopping transition for either backend. The in-process adapter passes the token directly to the executor, preserving cooperative cancellation and prior committed persistence effects. For an admitted scheduled `ChildWorker` handle only, the coordinator registers that handle's exact CTS before publishing the handle. Its callback captures that same handle, claims the existing stop ownership, and starts attached termination only for the first claim. It shares the same control owner and first monotonic stop deadline with the existing manual Stop and shutdown paths; it does not call `Cancel` recursively, look up a current active handle, or create another owner. The child adapter only delegates to the existing `WorkerRunControlPlane`. The existing coordinator/session cancellation owner and classifier/finalizer retain exact-session cancellation, grace/containment, exit/stdout/stderr drainage, evidence finality, classification, and cleanup; the adapter adds no cancellation registration or duplicate reporting.

Cancelling a wait is not cancelling the run. Raw exit 130, EOF, or a kill does not independently authorize `Cancelled`; the exact-session control intent and classifier precedence remain authoritative. A valid terminal committed by the bridge stays authoritative even if transport finality follows later. During cleanup, the coordinator settles the child, asynchronously detaches the linked scheduled-token registration, disposes the CTS, disposes the backend scope, and then releases the exact handle. The coordinator may expose the already-planned manual-dispatch and scheduled-await shapes, but both observe the same active handle and normalized terminal result.
