...
- Add an atomic, fail-fast lightweight cache-maintenance reservation on block 50's existing `ExclusiveHeavyWorker` ownership boundary; hold it for the complete per-cache or source-specific Delete All operation.
...
- Replace silent deletion helpers with typed per-target outcomes, idempotent missing-file behavior, safe permission/I/O failures, and truthful Delete All summaries for empty, invalid-only, mixed, duplicate, complete, and partial target sets.
...
The shared Web worker-job resource coordinator and safe busy consumers, a new lightweight page-independent cache-deletion command/storage seam, Administrative Areas deletion controls, obsolete source-service deletion entry points, the existing Data-page explicit reload/presentation path, operator documentation, CI category filters, and deterministic control-plane/filesystem tests are affected. Block 51's worker Ensure/Refresh/protocol/ownership behavior remains unchanged; block 53 later observes/adapts the finalized deletion results for its own inventory snapshot and is not a block 52 dependency.
