...
See `proposal.md` for motivation and `specs/worker-cache-operations/spec.md` for behavior. Block 47 finalizes v2 around one envelope-level `JobId`, case-sensitive closed `jobKind`, concrete request/event/result variants, common log/activity/terminal frames, a startup-validated typed handler registry, one host-owned terminal after acceptance, reusable session/cancellation/classification, and immutable arbitration metadata. It reserves `CacheMutation` but deliberately supplies no payload or handler. Finalized block 50 landed one singleton process-local `WorkerJobCoordinator` and `ExclusiveHeavyWorker` slot, `IWorkerJobAdmissionGate`, exact first-wins `Admitted(IWorkerJobAdmissionLease)` / `Busy(safe active snapshot)` / `Unavailable(safe pre-launch reason)` outcomes, safe `CacheMaintenance` category and `Manual` interactive origin metadata, monotonic Admitted/Starting/Running/Stopping/Finalizing lifecycle, owner-only cancellation/release, permanent shutdown fencing, and release after classifier plus process/stdout/stderr/protocol/bridge finality. It adds no queue, retry, preemption, priority, fairness, starvation guarantee, distributed lock, or CacheMutation operation. Exit 3 remains exclusive to the processing PostgreSQL advisory lock.
...
Add one concrete v2 request equivalent to `CacheMutationRequest(Source, Operation, Iso3)`, selected by envelope kind `CacheMutation`. `Source` is exactly `Overture` or `Gadm`; `Operation` is exactly `Ensure` or `Refresh`. Job identity remains only envelope-level. The descriptor declares landed `CacheMaintenance` capability, `Manual` interactive origin, cancellable=true, heavy=true, geodata-bearing=true, and block 50's `ExclusiveHeavyWorker` resource class. Add the concrete descriptor to immutable `WorkerJobDescriptors.Registered`; do not introduce parallel source names.
...
Block 51 replaces direct page mutation calls only. Existing status calls can remain temporarily as lightweight reads until block 53 extracts them, but no page/controller code may invoke download/export, DuckDB, GeoPackage conversion, geodata resolution, or local fallback. After every admitted attempt reaches authoritative owned finalization, including startup-unavailable, failure, and cancellation, the page explicitly reloads on-disk status; rejected Busy/Unavailable admission performs no reload. Define a narrow successful mutation-completed notification seam only for an authoritative completed/no-op result so block 53 can attach inventory invalidation without treating failed or cancelled work as success.
...
- [Applied block 50 source names differ from finalized planning labels] → Bind to the landed `WorkerJobCoordinator`, `ExclusiveHeavyWorker`, `IWorkerJobAdmissionGate`, `IWorkerJobAdmissionLease`, admission union, active snapshot, lifecycle, shutdown, and finality symbols; extend `WorkerJobDescriptors.Registered` without editing coordinator ownership or creating parallel names, DTOs, gates, identities, or release ownership.
...
1. Re-read applied block 47 and finalized/applied block 50 source. Bind to the exact landed `CacheMutation` reserved kind, envelope JobId/event/result/terminal adapters, `ExclusiveHeavyWorker` descriptor metadata, `WorkerJobCoordinator`, `IWorkerJobAdmissionGate`, `IWorkerJobAdmissionLease`, Admitted/Busy/Unavailable union, safe active snapshot, monotonic lifecycle, shutdown fence, classifier/process-stream-final release symbols, and `WorkerJobDescriptors.Registered`; stop on incompatible prerequisites rather than forking or renaming them.
