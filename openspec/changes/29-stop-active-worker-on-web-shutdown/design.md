...
- Edit finalized block 28 artifacts or its cancellation policy.

## Decisions

### Close admission in the lifetime callback and await cleanup in one hosted owner

Register the exact singleton coordinator/lifecycle owner with `IHostApplicationLifetime` during construction before it can be returned for admission. The synchronous `ApplicationStopping` callback calls a nonblocking `BeginShutdown`: under the coordinator's existing short gate it changes admission from open to stopping, captures the exact active handle, and publishes a memoized shutdown task before leaving the gate. It invokes no user callback, process I/O, or asynchronous continuation while holding the gate.

The same singleton is factory-aliased to one hosted lifecycle contract. Its `StopAsync` calls `BeginShutdown` again and awaits the already-published task. This makes `ApplicationStopping` the earliest admission fence and hosted `StopAsync` the sole asynchronous wait owner; they are not independent shutdown implementations. Callback disposal is idempotent. Registration failure prevents publication of the coordinator; partial `StartAsync` failure invokes the same fence/cleanup path.
...
Block 28 supplies the sole deadline: a fixed internal 10-second grace, measured by its injected `TimeProvider` from the first accepted Stop. Web composition does not override it. Instead, it determines an explicit reserve for one process-tree kill attempt, process exit, stdout/stderr finality, exactly-once disposal, and the stop-order cost of other hosted services, then configures/validates `HostOptions.ShutdownTimeout` to contain the remaining grace plus that reserve. If a Dashboard Stop began earlier, shutdown joins the already-elapsing task rather than restarting ten seconds. The applied reserve is 20 seconds, so `WorkerHostShutdownBudget` validates a minimum 30-second `HostOptions.ShutdownTimeout` before the coordinator factory resolves the executor or returns an admission owner. Larger valid budgets are preserved; infinite, shorter, and unsupported timer values are rejected. The coordinator is registered before `ProcessingBackgroundService`: Generic Host stops hosted services in reverse order, while `ApplicationStopping` has already fenced admission and started the shared worker deadline. The reserve covers that scheduler stop and subsequent process/resource cleanup; it is an allocation, not a guarantee that a failed platform kill will settle. Reference Compose files use `stop_grace_period: 40s` so Docker's outer stop timeout gives the host budget headroom.
...
Lifecycle setup proceeds in an order where the admission fence exists before the coordinator can be returned for admission. If hosted startup or later Web startup fails after the coordinator is constructed, its startup failure path or provider disposal transitions permanently to stopping and calls the same memoized cleanup. A constructor registration failure prevents the owner from being published at all. `StopAsync`, the application callback, and asynchronous disposal can repeat in any order; only the first transition captures ownership and all paths observe the same task. Callback registrations are disposed after cleanup and never reopen admission.
...
## Applied API reconciliation

- `ProcessingRunCoordinator.BeginShutdown()` publishes one `Task` under `_admissionGate`, captures the exact `ActiveRun`, marks it shutdown-owned, and captures `ActiveRun.ClaimStop(...)`. An asynchronous signal starts cancellation dispatch outside the gate. `StopAsync`, `DisposeAsync`, and synchronous `Dispose` join this same operation. The lifetime registration-ready signal handles an already-cancelled `ApplicationStopping` token invoking its callback before registration assignment finishes.
- `ActiveRun.CleanupCompleted`, the existing preparation ownership task, `TryAttachChildSession`, and the exact `ChildWorkerStopRequest` carry pending startup through attachment. `ChildWorkerSession.Settlement` covers physical exit, output finality, and resource finality; raw `Completion` alone is insufficient. No second process owner, pump reader, cancellation timer, or grace override is added.
- `ProcessingStateEventReporter.AbandonProjectedActivities(request)` is the existing nonterminal shutdown cleanup. It closes matching activities without releasing the projection arm, clearing running state, adding a fatal error, or creating a summary. Block 30 retains abnormal finalization authority. Ordinary non-shutdown preparation/execution faults keep the existing guarded `Abandon` behavior.
- A successfully completed owned cleanup can release the matching coordinator record while admission stays permanently closed. A failed tree kill with a live process leaves both session settlement and host shutdown pending.

Runtime references: [.NET 10 Generic Host shutdown](https://learn.microsoft.com/en-us/aspnet/core/fundamentals/host/generic-host?view=aspnetcore-10.0), [Host lifecycle source](https://github.com/dotnet/runtime/blob/v10.0.0/src/libraries/Microsoft.Extensions.Hosting/src/Internal/Host.cs), and [Compose stop grace period](https://docs.docker.com/reference/compose-file/services/#stop_grace_period).
