...
Finalized block 47 has a closed worker union of `ProcessAssets`, `CoordinateLookup`, and `CacheMutation`. The landed `WorkerJobCoordinator` owns one process-local `_gate`, `_active` owner, permanent shutdown fence, and fail-fast admission with no queue. Its `TryReserveCacheMaintenance` path uses a private exact-once `MaintenanceReservation`, and shutdown awaits that reservation's release. Block 54 extends this same concrete coordinator and reservation path for Database maintenance; it does not add a second gate, lifecycle owner, identity, or protocol kind. `IWorkerJobAdmissionGate` remains the worker/cache-facing contract and does not need a database method.
...
Only the four current database mutations enter this command. Reads remain direct bounded Web reads and do not reserve: reset option lists, skipped count, block-53 cache inventory, and Settings Test Connection. The reset pages consume narrow `ILocationValueOptionsReader` and `ISkippedAssetsCountReader` boundaries implemented by the existing repository singletons so component tests can witness real read timing and failures without replacing the mutation controller. Reads can overlap an active job because they neither publish nor delete shared geodata; their own connection disposal and failure presentation remain required.
...
Razor may parse for immediate feedback, but the command boundary is authoritative. Selected reset parses at the authoritative command boundary, preserves current behavior by ignoring and reporting malformed tokens when at least one valid GUID remains, rejects an empty valid set, and deduplicates the immutable valid set; it never silently ignores malformed tokens. Matching reset accepts only the closed City/State/Country discriminator and a nonblank bounded value while preserving the exact submitted database value, including leading or trailing whitespace and quotes. Reset-all carries explicit confirmed input, and the server handler refuses to create it unless the existing confirmation state is active; unconfirmed input is invalid rather than authorization. Clear Skip List accepts no path or database override.
...
For multi-store commands, order is PostgreSQL first, then skipped cleanup for the exact requested/returned target. If PostgreSQL fails, SQLite is NotStarted. For these one-statement repository mutations, a `PostgresException` whose nonlocalized invariant severity is exactly `ERROR` proves that PostgreSQL aborted the current command and may report atomic statement rollback. This is a narrow classification backed by PostgreSQL command-error semantics and an actual after-statement rollback test; `FATAL`, provider, transport, reader-disposal, and other exceptions do not prove commit state. An unknown-commit failure reports no fabricated zero count and safe copy that the Immich outcome was not confirmed; it does not authorize SQLite cleanup or retry. If PostgreSQL is confirmed committed and SQLite fails, the typed result is Partial and preserves the cleanup target inside an opaque result-owned capability. The UI offers an explicit **Retry skipped-list cleanup** action that reacquires maintenance independently, touches only `skipped.db`, and never repeats the committed Immich mutation. Targets are never rendered or logged as a raw ID list; UI shows bounded counts. For Reset All the retry target is All; selected uses all validated requested IDs even when PostgreSQL returns fewer changed rows; matching uses IDs returned before their values were nulled. No retry is automatic or queued.
...
The singleton maintenance controller installs its tracked Task before exposing admitted execution. Every connection, command, reader, and SQLite transaction is async-disposed/disposed before result finalization; `Pooling=false` remains for skipped SQLite and no global pool clear is introduced. The real immutable result is frozen, an optional timing witness may observe that point, then release runs exactly once in `finally`. Tracking cleanup removes only that exact old Task: reservation release can notify a reentrant observer that admits and starts a new operation before the old finalizer clears tracking, and the old finalizer must not overwrite or drop the new owner. Page reload occurs after release and is not part of protected mutation. A pure result mapper is not made replaceable solely to manufacture a mapping fault.
