...
See [proposal.md](proposal.md) and [specs/processing-work-detector-observability/spec.md](specs/processing-work-detector-observability/spec.md). Change 57 defines one advisory `IProcessingWorkDetector` call at the Standard scheduled pre-admission seam, with immutable request/result diagnostics and exceptional cancellation/failure semantics. Change 58 replaces its count-backed adapter with a current-full-eligibility existence query while retaining the exact Dashboard and worker counts. Apply must bind to the final landed change-58 strategy and must not alter archived change-58 artifacts or its finalized SQL/detector behavior.
