...
See `proposal.md` for motivation and `specs/processing/manual-child-worker-execution/spec.md` for behavior. At apply time, `Dashboard.razor` already injects `IManualProcessingRunCoordinator`: Run calls `TriggerManualAsync()`, Stop observes the shared `StopActiveRun()` task, and the page continues to observe `ProcessingState`. `ProcessingRunCoordinator` already owns local admission, request identity, cancellation, pending/reporting preparation, keyed backend dispatch, and matching-handle cleanup. The internal `AddWebComposition(context, ProcessingBackendKind.ChildWorker)` overload already provides the authorized explicit child-selection seam while the production overload remains `InProcess`.

This design consumes the finalized contracts from blocks 13, 25, 27, 28, 30, and 33 rather than recreating them. The coordinator preserves one `ProcessingRunRequest`/`Guid`; `ProcessingStateEventReporter` arms that exact request and owns its finalization receipt; `ChildWorkerProcessingRunBackend` delegates to `WorkerRunControlPlane`; `ChildWorkerSession` owns stream/process mechanics; the coordinator's active handle owns correlated Stop/grace/tree containment; and `WorkerRunFinalizer` plus `ChildWorkerEvidenceFinalityGate` reconcile evidence once. The existing deterministic seam names are `SessionTestProcess`, `SessionInputStream`, `SessionOutputStream`, and `CancellationTestClock`.
...
`Dashboard.razor` already uses the finalized block-13 Dashboard-facing manual start and active-run cancellation contract. Keep that control dependency on `IManualProcessingRunCoordinator`; do not inject a launcher, backend selector, child session, bridge, or classifier. `RunNow()` keeps its page-local pending guard and pre-trigger statistics refresh. The processing admission decision remains authoritative and manual contention remains silent.
...
2. create one immutable manual request with a non-empty run ID, live coordinator CTS, and active handle that captures the selected backend, then publish that handle;
3. call `ProcessingState.MarkPending()`;
4. arm the exact singleton reporter/state adapter for that request;
5. create one run scope and lazily resolve only the keyed backend captured on that handle;
...
Block 33's `TemporaryProcessingBackendSelection` remains internal and production Web composition defaults to `InProcess` in this numbered step. Use the existing authorized internal registration overload/test host builder to explicitly select `ChildWorker` for block-34 manual-path tests and transition verification. It uses the same production registrations except for that internal value; no string parsing or configuration binding is introduced. Scheduled behavior is not asserted or changed by this setup.
...
`ChildWorkerProcessingRunBackend` receives the coordinator dispatch contract but delegates only the exact coordinator and `ProcessingRunRequest` to the real `WorkerRunControlPlane`. The adapter adds no reporter forwarding or cancellation registration: the singleton reporter and the coordinator's active handle remain their respective owners. The control plane resolves the command, launches one process, starts stdout/stderr/exit observation immediately, waits for validated readiness, writes and flushes one execute request, and accepts typed events only for that run. Ready is transport-only; run-started confirms the already accepted execute/request correlation without resetting state; eligibility invokes the existing state-start/reset projection; absolute progress, activities, and diagnostics flow through the existing state adapter; the terminal projection/finalization receipt owns the sole Dashboard terminal mutation.
...
The manual coordinator path is exercised by `ManualChildWorkerControlPlaneTests` (admission, state projection, and early Stop), `FailureControlPlaneTests` (ready/execute failures, cleanup, and retrigger), and `ProcessFixtureManualCoordinatorTests` (the existing fixture’s terminal, cancellation, and forced-kill modes). Existing focused coverage in `ChildBackendLifecycleTests`, `CoordinatorStopTests`, Dashboard binding tests, and finality/protocol suites supplies the selection, cancellation, UI-boundary, and stale-event matrices without a duplicate harness.

After the independent review closed its process-fixture launch-count synchronization warning, the post-fix logs recorded 26/26 focused tests in `_out/change34-final-focused-tests.log`, 1,535/1,535 default tests in `_out/change34-final-default-tests.log`, a Release publish with zero warnings or errors in `_out/change34-final-publish.log`, and 305/305 published-worker tests in `_out/change34-final-published-worker-tests.log`. Strict OpenSpec validation and the scope review passed: no scheduled-block-35 implementation, public selector, production-default change, or fallback path was found.
