...
See `proposal.md` for motivation and `specs/immich-watermark-source-selection/spec.md` for the gate. The current repository batches eligible rows by `(asset."createdAt", asset.id)`, but that is pagination only; eligibility is city and country null, latitude and longitude present, and asset not deleted. ReverseGeo writes and all three clear paths update `asset_exif` without explicitly managing a timestamp. Block 58's implemented `SELECT EXISTS` remains a full current-state observation and is the safe baseline.

Upstream evidence was reviewed against Immich v3.1.0 ([release](https://github.com/immich-app/immich/releases/tag/v3.1.0), commit [`8aa95c67`](https://github.com/immich-app/immich/commit/8aa95c67470a02a8ddedf03c2e52963af33065ff)) and historical research commit [`469a870a`](https://github.com/immich-app/immich/commit/469a870a2233e7361bcb855b183fd41272cfd056). Implementation also inspected v3.2.0 commit `1b6098c9dbfffe978bec2d414606ed7a4c8e019a`; the pinned matrices and isolated proof are published in [maintainer research](../../../docs/maintainer/immich-watermark-source-research.md). The result is intentionally a no-go outside as well as inside that observed matrix: absence of evidence is not compatibility.
...
|---|---|---|---|---|
| `asset.createdAt` + `id` | Inserts only; repository uses it for batches | Deterministic tuple; asset ID is a stable secondary key | Misses delayed EXIF, GPS changes/clears, ReverseGeo clears, old-row restores; not commit order | Reject |
...
| Asset marker plus `asset_audit` | Soft/hard asset events plus EXIF current state | Separate streams | Hard-delete audit exists, but the reviewed model does not demonstrate a complete EXIF-delete audit or common commit order ([audit](https://github.com/immich-app/immich/blob/469a870a2233e7361bcb855b183fd41272cfd056/server/src/schema/tables/asset-audit.table.ts#L4-L16)) | Reject |
| `xmin`, txid, or transaction snapshots | Surviving row versions | PostgreSQL internals, not stable application index | XIDs allocate before commit; internal 32-bit xid wraps, and current rows do not retain deleted/transient history ([system columns](https://www.postgresql.org/docs/16/ddl-system-columns.html), [snapshots](https://www.postgresql.org/docs/16/functions-info.html#FUNCTIONS-PG-SNAPSHOT)) | Reject |
| Trigger + `LISTEN/NOTIFY` | Configurable events | No durable queue | Session registration is transient and initialization races; notification is wake-up only ([LISTEN](https://www.postgresql.org/docs/16/sql-listen.html), [NOTIFY](https://www.postgresql.org/docs/16/sql-notify.html)) | Reject |
...
| Durable logical decoding slot + commit LSN | Committed INSERT/UPDATE/DELETE from both relations | Commit-ordered WAL feed | Conditionally safe only with bootstrap, retention, replay, failover, privileges, and DDL controls ([slots](https://www.postgresql.org/docs/16/logicaldecoding-explanation.html#LOGICALDECODING-REPLICATION-SLOTS), [ordering](https://www.postgresql.org/docs/16/logicaldecoding-output-plugin.html#LOGICALDECODING-OUTPUT-PLUGIN-CALLBACKS)) | Future research candidate, not block 62's polling design |
...
|---|---|---|
| Older naming/schema revisions | Not tested; the migration below is only a naming witness | Outside verified watermark matrix; do not infer compatibility |
| Rename commit (release interval not verified here) | Rename introduced by commit [`c699df00`](https://github.com/immich-app/immich/commit/c699df002a32ac175cee276b8f7a9eab1e1b4c42) ([migration](https://github.com/immich-app/immich/blob/c699df002a32ac175cee276b8f7a9eab1e1b4c42/server/src/schema/migrations/1752267649968-StandardizeNames.ts#L5-L18)) | Names alone do not prove marker semantics or index compatibility; fail closed |
...
| Historical main reference / `469a870a` | Same reviewed model at research time | Research reference only, not a supported release contract; all scalar candidates fail |
| v3.2.0 / `1b6098c9` | Asset/EXIF/audit table models and relevant marker/upsert blocks match the reviewed evidence | Scalar candidates still fail; source inspection is not an operational support contract |
| Any future schema | Unknown until pinned and tested | Stop incremental advancement; use EXISTS only if its own schema contract remains valid |
...
The standalone synthetic research script creates and removes only a uniquely named database in a dedicated disposable PostgreSQL container. It demonstrates commit inversion for timestamp/UUID/XID/xmin scalars and tuples, including equal markers; future mutation/recovery cases remain defined acceptance gates, not claimed executed integration coverage.
