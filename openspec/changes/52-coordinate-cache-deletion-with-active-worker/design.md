...
At apply start, bind to the exact landed block 50 coordinator/resource gate. Extend that same atomic `ExclusiveHeavyWorker` owner boundary with a non-worker Cache maintenance reservation; do not query the active snapshot and then delete, wrap the coordinator in a second lock, or create a parallel semaphore. Acquisition returns a closed Reserved(owner handle), Busy(safe active owner), or Unavailable(safe shutdown/configuration reason) result. First atomic winner owns the slot; losers fail immediately with no wait, queue, retry, preemption, priority, fairness, or starvation promise.

A maintenance owner has safe category Cache maintenance and an explicit cache-UI origin so later worker requests can receive truthful Busy metadata. It has no `WorkerJobKind`, JobId, PID, process lifecycle, cancellation capability, terminal, protocol frame, or exit code. Preserve block 50's canonical JobId and worker snapshot invariants for worker owners by representing the resource owner and safe busy/diagnostic projection as closed worker-or-maintenance shapes rather than inventing a fake CacheMutation job. Existing processing, Lookup, cache-refresh, diagnostics, and test-gate consumers may be adapted narrowly to this shared projection, but block 51's Ensure/Refresh request, protocol, worker lease, cancellation, and finality semantics do not change. Every admission-gate decorator forwards maintenance admission to the same coordinator; worker-only test gates reject unexpected maintenance use instead of fabricating ownership. The opaque admitted handle remains the exact-once release capability.
...
Use a closed Overture/GADM source plus exact `^[A-Z]{3}$` ASCII input, the lightweight bundled country identity catalog, and source mapping (including the established GADM alias policy). The current mappings are total over the bundled known-country catalog: Overture uses the catalog identity and GADM is identity except `XKX` to `XKO`; the final GADM cache name remains the application identity such as `XKX.db`. Keep rejecting any input whose actual mapping cannot produce a canonical source code, but do not claim an unreachable known-country unmappable branch. Do not normalize user input. Pure source/identity/mapping validation occurs before admission, so an invalid request remains Invalid even when the coordinator is Busy or shutdown-fenced. Only after validation and successful reservation derive and inspect the fixed source root and `{ISO3}.db`; accept no path, URL, or file name from Razor or any status row.

Inspect the configured component sequence, including the filesystem root, before lexical normalization can erase a linked component followed by `..`; then canonicalize the source root and target, require target containment using platform-appropriate path comparison, and inspect the final target for link/reparse metadata immediately before deletion. Reject a linked configured root or final entry and never recursively traverse. The storage root is operator-controlled; this does not claim to defeat a hostile process swapping filesystem entries between checks, which belongs with a future OS-level/distributed storage lock.

Delete only the final database. Do not preserve the current broad `{ISO3}.*.tmp` deletion: block 51 owns unique candidates and cleanup, and another container may own a matching live candidate despite the local reservation. Once the page is migrated and graph/source checks confirm no other production caller, remove the obsolete public source-service `DeleteFile` entry points and retire or rework only their obsolete tests; retain independently used internal cleanup and all Ensure/Refresh behavior.
...
Replace void/silent `DeleteFile` behavior in the Web path with typed `Deleted | Missing | Invalid | Failed` outcomes. Missing is idempotent success of the desired absent state and comes only from an explicit missing-file/directory result at the narrow filesystem boundary; do not infer absence from `File.Exists(false)`, which can hide access and I/O failures. Map expected permission, read-only, sharing/in-use, directory, and I/O errors to bounded codes/copy that omit host paths, caller path-like input, and raw exceptions. The application has no general-purpose exception redaction provider, so log only safe structured source/canonical ISO3, a fixed inspect/delete phase, exception type, and HResult; do not attach the exception or copy its message/stack. Unexpected programmer/cancellation failures still unwind through `finally` and release.

Delete All is source-specific and takes one immutable list of typed source/ISO identities from the current page state. It validates every identity before admission, treats the first occurrence of a valid source/ISO3 identity as eligible, and records later duplicates as Invalid so they do not become automatic retries. An actually empty input returns zero counts with no admission or storage inspection. A nonempty invalid-only input returns ordered Invalid results/counts with no admission or storage inspection. A mixed input preserves its Invalid preflight results and then acquires one reservation for its unique valid targets. If admission is Busy or Unavailable, it returns the truthful preflight Invalid results plus the eligible/unattempted count and fabricates no target outcome. If reserved, it attempts each eligible target independently in deterministic ISO3 order under the one reservation, combines the ordered results, and never stops at the first ordinary filesystem failure or reports the pre-operation count as deleted.
...
The deletion command completes every requested filesystem attempt under the reservation, freezes the single-target or ordered batch `Deleted | Missing | Invalid | Failed` result, then releases its owner handle in `finally`, and returns only after release. It neither knows about nor calls an inventory service, cache, invalidation event, snapshot, or reload seam. Finalized results are the stable later integration point; block 53 may observe/adapt them to invalidate its own snapshot without changing deletion semantics. The command never triggers a storage status reload itself.

After any Completed deletion outcome, and after ownership release when a reservation was taken, the existing Data-page flow performs its current explicit status reload and derives presentation from the finalized result plus newly read disk state. The page keeps conflicting controls disabled through that reload. The coordinator may admit another worker during this lightweight reload because deletion ownership has ended. Busy/Unavailable attempts do not reload storage. A page reload failure is separate page/read-path behavior, not a deletion result, and cannot rewrite a Deleted outcome or restore a file.
...
5. Preserve the existing page-owned explicit reload after any Completed outcome and after release when reserved, add operation-generation, source-named pre-admission confirmation for per-cache and Delete All requests, and accessible finalized-outcome UX with no in-operation Delete Cancel, then add composition tests proving no heavy Web service, worker launch, protocol, processing state, global pool clear, or temporary cleanup.
6. Add the Change52 category to the existing three-OS worker build-and-published CI filters without changing their jobs or adapters. Run focused Web/source/platform tests, normal tests, docs build, strict OpenSpec validation/status, and a scope review proving block 51 worker behavior and block 53 artifacts/implementation were not changed. Rollback restores the prior page call path and removes the maintenance owner extension together; no data/config migration is required.
