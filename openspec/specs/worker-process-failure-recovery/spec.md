# worker-process-failure-recovery Specification
...
- **THEN** no PID or ready/terminal event is fabricated, ProcessAssets finalizes one Failed control-plane outcome while CoordinateLookup and CacheMutation finalize their existing Unavailable page outcome, launch/classification telemetry uses the established safe category, coordinator ownership is released exactly once, and no child exists
...
- **THEN** no new job is admitted, shutdown joins the same cancellation/deadline/kill operation, and clean host completion is not reported until the child tree, streams, bridge, disposal, and owner release settle; the test separately joins best-effort telemetry observations with a bounded nonblocking recording sink and MUST NOT require production shutdown or process control to wait for arbitrary logger delivery

### Requirement: Protocol corruption fails closed while drains remain live
The suite SHALL inject malformed JSON, invalid UTF-8 or framing, EOF-truncated JSON, a line over 1,048,576 bytes, unknown semantic protocol/version/direction/category/type values, duplicate properties, payload invariant failures, wrong correlation, sequence gaps/regressions/duplicates, illegal lifecycle order, post-terminal frames, and stdout non-protocol text. It SHALL separately prove the existing version-specific compatibility rules: otherwise valid v1 frames with additive unknown envelope or payload properties are accepted and projected using only known fields, while v2 frames with unknown envelope or payload properties are rejected by their closed field sets before projection. This matrix MUST NOT widen either codec contract. The first typed protocol fault SHALL be retained, invalid frames SHALL cause no projection, and fault containment SHALL continue draining both streams before one Failed finalization.
...
- **THEN** v1 accepts and projects the known event, omits the additive property from canonical reserialization, and emits no protocol violation; v2 rejects an unknown envelope property as InvalidEnvelope or an unknown payload property as InvalidPayload before invalid projection and emits the corresponding safe 6630 observation
...
- **THEN** a row with zero enqueue waits and zero replacements emits no 6650, while any row with an enqueue wait or replacement emits exactly one 6650 containing only the finalized bounded aggregate fields; the matrix includes a deliberate replaceable-progress pressure row and permits lossless FIFO pressure to satisfy the same catalog condition
