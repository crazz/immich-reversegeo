# processing-run-executor-testing Specification
...
#### Scenario S01: Standalone deterministic fixture
...
#### Scenario S02: Direct extraction test already covers the behavior
...
#### Scenario S03: Host and composition extraction tests remain outside the fixture
...
#### Scenario S04: Authoritative count is zero
...
#### Scenario S05: Non-empty run takes one snapshot of each input
...
#### Scenario S06: Suppressed asset is fetched
...
#### Scenario S07: Multiple non-empty batches reach an empty sentinel
...
#### Scenario S08: Delay follows each non-empty batch
...
#### Scenario S09: Eligibility total differs from fetched work
...
#### Scenario S14: Successful location update
...
#### Scenario S15: No country or administrative match
...
#### Scenario S16: Country fallback prevents a logger-only no-city skip
- **WHEN** a matched administrative result reaches fallback with neither City nor State
- **THEN** Country is assigned to City, the normal update path is used, no skipped-store insert, Skipped disposition, or warning is fabricated, and the logger-only no-city guard remains unreachable

#### Scenario S17: Ordinary per-asset source or persistence operation fails
...
#### Scenario S18: Reporter fails from inside resolver reporting
...
#### Scenario S19: Airport lookup is disabled
...
#### Scenario S20: Containing airport overrides administrative city
...
#### Scenario S21: Non-containing airport does not override administrative city
...
#### Scenario S22: Non-containing airport fills an absent administrative city
...
#### Scenario S23: Administrative fallback order
...
#### Scenario S24: Persistence fails before disposition
...
#### Scenario S25: Active cancellation is observed during persistence
...
#### Scenario S26: Cancellation follows successful persistence
...
#### Scenario S27: Cancellation follows a committed handled-failure decision
- **WHEN** cancellation is requested after a reachable handled Failed decision is committed but before its disposition publication is accepted
- **THEN** Failed publication completes through the established non-cancelled committed path and the resulting terminal counts retain that disposition

#### Scenario S28: Later work cancels or fails
...
#### Scenario S29: Reporter fails after persistence
...
#### Scenario S30: Cancellation before eligibility succeeds
...
#### Scenario S31: Cancellation before asset disposition
...
#### Scenario S32: Cancellation between batches or during delay
...
#### Scenario S33: Foreign cancellation-like exception
...
#### Scenario S34: Pass-level repository, snapshot, or delay failure
...
#### Scenario S35: Per-asset repository failure
...
#### Scenario S36: Critical out-of-memory failure in execution
...
#### Scenario S37: Reporter fails while opening the run and emitting start
...
#### Scenario S38: Reporter fails during a midstream report
...
#### Scenario S39: Reporter rejects finish
...
#### Scenario S40: Completed result includes mixed dispositions
...
#### Scenario S41: Cancelled or failed result retains partial counts
...
#### Scenario S42: Fixed timestamps and terminal order
...
#### Scenario S43: Eligibility and processed counts diverge
