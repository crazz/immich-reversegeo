...
## 2026-08-30 — Change 12: pre-edit implementation contract

- Change-12 start UTC: **2026-08-30T22:36:46Z**.
- Baseline: branch `major-redesign`; HEAD and `origin/major-redesign` both `472a9856811b24d7210aaa4b3543aa9107669c4b`. The deleted active Change-02 files and the user-listed untracked archive/spec/tooling paths are unrelated preserved baseline and must never be staged or modified.
- Prerequisite evidence: source exposes final `IProcessingRunExecutor.ExecuteAsync(ProcessingRunRequest, IProcessingEventReporter, CancellationToken)`; `ProcessingRunExecutor` owns OpenRun → exact `GetUnprocessedCountAsync` → eligibility → zero gate → skipped/config snapshots → cursor batches → resolver/infrastructure → persistence/dispositions → terminal result. `ProcessingBackgroundService` owns startup, admission, MarkPending, request/reporter arming, manual CTS, executor dispatch/abandon, and lock release. `AddProcessingServices` registers `TimeProvider.System`, singleton executor/interface alias, singleton concrete host, and hosted alias factory. Focused prerequisite command `dotnet test --project tests/ImmichReverseGeo.Tests/ImmichReverseGeo.Tests.csproj --filter "FullyQualifiedName~ProcessingRunExecutorChange11Tests|FullyQualifiedName~ProcessingBackgroundServiceDelegationTests|FullyQualifiedName~ProcessingServiceRegistrationTests" --no-restore` passed **31/31**, 0 failed/skipped, MTP 10.842s.

### Frozen host/startup/schedule/admission/manual operation map

- Startup: ILogger “initialising skipped-assets db” → await the exact singleton repository `InitialiseAsync` → ILogger ready/elapsed → UI `Service started. Waiting for next scheduled run.` → first config read. Startup itself creates no request/run.
- Schedule iteration: one `GetConfigAsync` snapshot → disabled one-minute wait, or Cronos Standard UTC strictly-future calculation → invalid/no-next five-minute wait, or one exact `Next run scheduled at {due:u}` UI line before positive relative wait → one scheduled admission attempt → accepted execution awaited to terminal or exact contention line → next config read. Saves do not interrupt any active wait or accepted execution.
- Admission/execution: nonblocking `_runLock` → immediate `MarkPending` → unique Scheduled/Manual request → exact reporter `Arm` → executor once → reporter abandonment only on escaping executor failure → pending cleanup/release. Rejection owns no request, reporter session, executor call, or accounting.
- Manual: `TriggerRunAsync` keeps nonblocking lock, pending, Manual identity, private manual CTS, fire-and-forget task and fault continuation; `CancelRun` cancels only that manual CTS; `WaitForManualAdmissionAsync` remains a test seam. Dashboard concrete injection remains compatible.
- Dependency guard: schedule plans/loop may depend only on copied Enabled/Cron schedule values, `TimeProvider`, UI visibility, startup initialization, and the minimal Scheduled trigger boundary. They must not depend on asset/skipped repositories, eligibility/count, batches, resolver/geodata, persistence, executor internals, ProcessingState admission inspection, coordinator/work-detector/protocol types, or ScheduleEditorState parsing. No preflight or data-plane dependency is permitted anywhere in scheduling.

### Requirement-to-executable-test matrix (pre-edit)

| Requirement/task | Production behavior | Exact planned executable test method | Exact identity/order/accounting/failure expectation | Stable gates/cancellation points | Coverage kind |
|---|---|---|---|---|---|
| 1.1 executor prerequisite | Executor remains sole exact eligibility/data-plane owner | `ExecuteAsync_ZeroEligibility_UsesOneSessionBeforeCountAndReturnsExactCompletedResult` | same request; open→count→Eligibility(0)→Finished; 0/0/0/0; no downstream calls | existing count/event gates; every task bounded | prerequisite/identity |
| 1.2 host prerequisite | Freeze startup/retry/UTC/log/contention/await/manual behavior | `ExecuteAsync_CharacterizedHostSequence_RemainsExactAfterScheduleExtraction` | exact startup/config/log/trigger/admission sequence; no extras | initializer/config/plan/trigger TCS, all asynchronous and bounded | characterization/order |
| 1.3 dependency guard | Schedule types expose no data-plane/control-plane dependencies | `ScheduleTypes_ExposeOnlyClockSnapshotVisibilityAndTriggerDependencies` | reflection finds zero repository/count/skipped/resolver/geodata/executor/coordinator dependencies | no timing; bounded test task | architecture |
| 2.1 plans | Immutable DisabledRetry, InvalidRetry, Due | `Calculate_ReturnsExactImmutablePlanShapesAndDurations` | disabled=1m; invalid=5m; Due exact UTC instant/offset; no mutable properties | fixed values only | unit/value |
| 2.2 Cronos | Standard five-field, UTC, strict future, invalid/no-next | `Calculate_FixedUtcExpressions_UseStandardStrictlyFutureUtcSemantics` | hourly/daily/weekly/custom exact instants; current match excluded; six-field rejected | fixed UTC data rows | unit/boundary |
| 2.3 clock/waits | One provider supplies now and cancellable delay; nonpositive due emits once | `ExecuteAsync_UsesInjectedClockForPlanAndWaitAndEmitsAlreadyDueOnce` | clock call sequence exact; no real delay/log for passed due; one Scheduled trigger | controllable provider call gates; bounded trigger task | fake-time/race |
| 3.1 trigger contract | Minimal Scheduled-only async result and hosted token | `ScheduledTriggerBoundary_ReportsRejectedOrAcceptedAfterTerminalWithExactToken` | exact Scheduled identity; exact token; only two result values; acceptance completes after terminal | per-call entered/terminal TCS | contract/identity |
| 3.2 adapter/no cycle | Exact old host path wrapped without moved ownership/back-edge | `AddProcessingServices_HostAndScheduledBoundaryResolveWithoutCycleAndShareIdentity` | concrete/IHostedService ReferenceEquals; boundary resolves; no adapter field/ctor back-edge to host | bounded provider resolution | DI/architecture |
| 3.3 rejection/acceptance | Rejection zero executor and exact line; acceptance pending once and blocks | `TryTriggerScheduledAsync_RejectionAndAcceptancePreserveExactAdmissionSemantics` | rejection exact contention only/0 executor; accepted same request/reporter/token, 1 executor, terminal then release | lock owner, executor entered/release, terminal TCS | concurrency/accounting |
| 3.4 no coordinator | No block-13 owner/API/metadata/cancellation consolidation | `ScheduleTypes_ExcludeCoordinatorAndManualControlPlaneSurface` | reflection finds no coordinator/work-detector/public trigger metadata or manual API on schedule types | none | scope/architecture |
| 4.1 startup | Initialize then ready ILogger then service UI line then config | `ExecuteAsync_StartupInitializationCompletesBeforeServiceLogConfigAndTrigger` | exact full sequence; zero config/log/trigger before release; no startup run | initializer entered/release, service-log and config TCS | lifecycle/order |
| 4.2 loop | One snapshot→plan→optional exact log→fake wait→one trigger | `ExecuteAsync_PositiveDue_LogsExactUtcLineThenWaitsAndTriggersOnce` | exact due:u text precedes wait; one Scheduled identity; no duplicates/extras | plan/config/log/delay/trigger TCS; fake-time advance | fake-time/order |
| 4.3 reevaluation | Saves pinned until existing reevaluation point | `ExecuteAsync_ConfigChangesRemainPinnedUntilEachPlanCompletes` | disabled/invalid/due snapshots read once; new value only after 1m/5m/due handling | versioned config-read TCS and fake-time gates | race/config |
| 4.4 shutdown/manual | Shutdown cancels waits/accepted run; manual APIs unchanged | `ExecuteAsync_ShutdownCancelsEveryWaitAndAcceptedRunWithoutPostCancellationTrigger` | exact hosted token; no ordinary error/post-cancel trigger; manual token remains separate | retry/due/accepted entered TCS; cancel at each checkpoint | cancellation/lifecycle |
| 4.5 no work preflight | Every accepted due reaches executor including zero | `TryTriggerScheduledAsync_AcceptedZeroWorkInvokesExecutorAndPublishesAuthoritativeZero` | one Scheduled request; executor count/Eligibility(0)/Finished exactly once; schedule zero data calls | trigger/executor count/terminal TCS | boundary/accounting |
| 4.6 host identity | Singleton concrete/hosted alias and Dashboard surface retained | `AddProcessingServices_ConcreteHostedAndDashboardServiceAreReferenceEqual` | ReferenceEquals concrete/IHostedService; one hosted descriptor; manual methods callable | bounded resolve/start/stop | DI/compatibility |
| 5.1 calculator matrix | Exact UTC hourly/daily/weekly/custom/invalid/no-next/local-zone independence | `Calculate_HourlyDailyWeeklyCustomAndInvalidMatrix_IsUtcAndHostZoneIndependent` | every expected instant has offset zero; same output under changed local-zone context; no host-now read | fixed inputs, serialized environment restoration | unit/matrix |
| 5.2 retries | Exact 1m/5m, no trigger/next-run log, fresh read only after advance | `ExecuteAsync_DisabledAndInvalidRetryWaitExactDurationsBeforeFreshRead` | config→1m or 5m delay; zero trigger/next-run; second read only after exact advance | per-read/per-delay TCS; fake time; bounded waits | fake-time/negative |
| 5.3 due edges | Future logs/triggers once; passed due no wait/log; shutdown none | `ExecuteAsync_FutureAlreadyDueAndCancelledDuePlansHaveExactSingleTriggerSemantics` | three rows with exact UTC/log counts; zero duplicate/post-cancel calls | wait-start/trigger TCS and cancellation-before-due | fake-time/boundary |
| 5.4 adapter gating | Accepted blocks reevaluation; rejection zero executor; shutdown token reaches executor | `ExecuteAsync_AcceptedTriggerBlocksReevaluationAndPropagatesShutdownToken` | config calls stay 1 until terminal; rejected exact line/0 call; accepted executor sees identical token | admission/entered/release/second-read TCS | concurrency/cancellation |
| 5.5 config pinning/editor | Disabled/invalid/due changes wait; editor behavior untouched | `ExecuteAsync_ConfigMutationDuringEveryWaitAppliesOnlyNextIteration` | exact version and cron consumed per iteration; no reschedule/wake | immutable snapshot queue, fake time, read TCS | race/compatibility |
| 5.6 hosted lifecycle | Startup ordering, no immediate run, cooperative stop | `StartAsync_StopAsync_GatedStartupAndScheduleTerminateCooperatively` | initialize→service log→config; strict-future only; Stop completes; no extra trigger | startup and wait TCS; bounded Start/Stop tasks | lifecycle |
| 5.7 empty execution/layering | Zero-work accepted trigger still executes; schedule has zero data calls | `TryTriggerScheduledAsync_ZeroWorkPreservesExecutorEligibilityAndScheduleLayering` | exact one executor/open/count/eligibility/finish; zero batch/geodata/persistence and zero scheduler repository access | executor event TCS | integration-unit/boundary |
| 5.8 manual inheritance | Pending/cancel/exclusion/Run Now stay exact | `TriggerRunAsync_Accepted_DelegatesExactArmedRequestReporterAndManualTokenOnce` | exact Manual request/reporter; CTS cancellation; one call; pending cleared/lock reusable | existing asynchronous executor gates; bounded | regression/manual |
| spec local-zone scenario | Explicit zero-offset input rejects/normalizes nonzero and never host DST | `Calculate_NonZeroOffsetInput_IsRejectedAndUtcInputIgnoresLocalDst` | deterministic boundary result; zero output offset; identical UTC calendar result | fixed instants | unit/negative |
| spec no-next scenario | Parse success with no possible next is InvalidRetry | `Calculate_ExpressionWithoutNextOccurrence_ReturnsFiveMinuteInvalidRetry` | exact InvalidRetry(5m), no Due | fixed maximum UTC instant | unit/boundary |
| spec retry shutdown | Cancellation during each retry exits quietly | `ExecuteAsync_ShutdownDuringDisabledOrInvalidRetryProducesNoTriggerOrError` | zero triggers/next-run/error and no fresh read after cancellation | retry-wait entered TCS; cancel token; bounded execution | cancellation/negative |
| spec positive visibility | One exact UI next-run line only before positive wait | `ExecuteAsync_PositiveWaitPublishesOneExactUtcNextRunLineBeforeDelay` | complete log array contains startup then exact next line, no duplicate; trigger only after advance | log-added and delay-start TCS | visibility/order |
| spec scheduled rejection | Existing owner causes exact contention visibility | `ExecuteAsync_DueRejected_AppendsExactContentionLineAndDoesNotExecute` | startup/next-run/contention full array exact; zero rejected executor/request/accounting | owner held/release and trigger-result TCS | admission/negative |
| spec accepted terminal | Scheduler waits terminal before reread | `ExecuteAsync_DueAccepted_DoesNotReadConfigAgainUntilTerminal` | one read/trigger/executor while gated; second read only after terminal/release | executor entered/terminal and config-read TCS | concurrency/order |
| 6.1 focused gate | All change/relevant retained classes executable together | `Change12Matrix_AllNamedProofMethodsExist` | reflection enumerates exact planned methods, preventing evidence-only claims | no timing | meta/coverage |
| 6.2 full compatibility | Settings/config/editor and inherited lifecycle remain green | `SettingsAndScheduleEditor_PublicBehaviorRemainsUnchanged` | exact persisted fields/preset/custom text and no added fields | existing deterministic tests | regression |
| 6.3 strict/status | Artifacts remain valid and eventually 29/29 | `Change12Tasks_HaveOneExecutableProofMappingPerRequirement` | task IDs/method mapping unique and complete | no timing | meta/spec |
| 6.4 scope/dependencies | No blocks 1–11/13+, UI/query/geodata/protocol changes | `ScheduleTypes_ExposeOnlyClockSnapshotVisibilityAndTriggerDependencies` | forbidden constructor/field/interface type set is empty | reflection | scope/architecture |
| 6.5 replacement point | Temporary boundary documents block-13 semantic obligations | `ScheduledTriggerBoundary_ContractEncodesBlock13ReplacementSemantics` | Scheduled only; rejection vs accepted-after-terminal; clock/wait/config/log remain upstream | gated accepted completion | contract/future-proofing |

### Deep failure-boundary preflight (pre-edit)

| Boundary/state | Propagation | Cleanup owner | Loop usability / session status | Logging and trigger permission | Irreversible accounting/admission point | No-recursive-reporting rule |
|---|---|---|---|---|---|---|
| Success | plan returns or accepted run returns terminal | TimeProvider delay disposes registration; downstream host releases lock/session | loop usable; session healthy→finished | exact startup/positive-next-run/optional contention only; one due trigger | accepted lock+MarkPending; executor dispositions remain authoritative | scheduler never reports executor terminal recursively |
| Active host cancellation during disabled retry | `OperationCanceledException` filtered by stopping token exits ExecuteAsync | delay/token registration | loop terminates cooperatively; no session | no trigger, next-run, contention, or ordinary error | none | no reporter involved |
| Active host cancellation during invalid retry | same cooperative exit | delay/token registration | loop terminates; no session | no trigger/log/error | none | no reporter involved |
| Active host cancellation during positive due wait | same; due trigger suppressed | delay/token registration | loop terminates; no session | existing next-run may remain; no trigger/contention/error after cancellation | no admission | no reporter involved |
| Active host cancellation while accepted run awaited | exact hosted token reaches old host/executor path; executor classifies per block 11 | downstream executor/session then host lock release | loop terminates after accepted call; healthy terminal unless reporter fails | no next occurrence/post-cancel trigger; executor owns run events | admission already irreversible at lock+pending+armed request; prior executor dispositions persist | host only abandons on escaping failure; no scheduler terminal reporting |
| Post-due pre-trigger cancellation | check stopping token immediately before trigger; suppress trigger | scheduler | loop terminates | no trigger or contention | none | n/a |
| Post-admission cancellation | accepted path remains accepted-after-terminal; token propagates | existing host/executor/reporter/finally | loop cannot reevaluate; lock released once; session terminal or broken | no new trigger; only downstream permitted events | request/pending/arm already irreversible; accepted accounting remains executor-owned | no duplicate abandon/finish |
| Foreign cancellation-like calculator/config/trigger failure | ordinary failure, not host shutdown; propagate/fault hosted service under existing BackgroundService policy | owning collaborator; host finally if admitted | loop faulted; session absent or downstream-defined | no synthetic cancellation, retry, or recursive UI failure line | none before admission; admitted effects preserved | scheduler does not convert/report recursively |
| Reporter/infrastructure failure downstream | original exception escapes exact old RunOnce/Abandon path | reporter/session and host admission finally | session broken; loop faults after lock release | no further session logging/terminal; no future trigger | prior writes/dispositions and admission persist | absolute: no scheduler reporting or second terminal attempt |
| Ordinary config failure | propagates as hosted-loop failure; no retry invented | config source | loop faulted, no session | no trigger/next-run/contention after failure | none | no reporter involved |
| Ordinary calculator failure | only invalid cron/no-next maps to InvalidRetry; unexpected infrastructure exception propagates | calculator | retry only for classification; otherwise loop faulted | invalid emits no line/trigger; unexpected no synthetic log | none | no reporter involved |
| Ordinary trigger failure | propagate after exact adapter cleanup | existing host control path | loop faulted; downstream session status preserved | no duplicate trigger/contention/error invented | admitted request/pending/arm may be irreversible; release once | existing Abandon once only; scheduler never retries reporting |
| OutOfMemoryException | never caught as invalid schedule or ordinary loop error; propagates | current owner; host finally if admitted | loop unusable/faulted; session downstream status | no synthetic UI error/trigger retry | prior admission/accounting persists | never report recursively or downgrade |
| Cleanup failure | original failure precedence follows unchanged host/executor boundary; schedule delay cleanup must not mask active cancellation | delay registration or downstream session/host | loop faulted if cleanup itself is primary; broken session remains broken | no later trigger/log/terminal retry | prior effects persist; lock release attempted exactly once | no recursive cleanup reporting |
| Admission rejection | result RejectedAlreadyRunning; no request/executor/session | current run owner retains lock | scheduler loop usable and reevaluates after immediate result | exact contention line once; no execution trigger effects | rejected call has none | n/a |
| Broken downstream session | original reporter exception by reference; adapter releases lock | reporter/host | broken; scheduler does not repair; hosted loop faults | no further run events or schedule retry | prior accepted events/persistence remain | absolute prohibition on scheduler/reporting recursion |
| Stale/cross-run/duplicate trigger operation | boundary creates one fresh Scheduled request only after admission; old reporter correlation rejects downstream | host/reporter | current valid owner remains; invalid correlation broken/rejected | no second scheduler emission for same plan; no fabricated contention unless admission rejects | first successful admission only | never retry stale operation through same session |

- This record, matrix, operation map, dependency guard, and failure preflight were appended **before any Change-12 production or test code edit**. Only this implementation log has changed so far.

### Change 12 completion evidence

- Completion UTC: **2026-08-30T22:50:17Z**; elapsed from mandated start: **13m 31s**. Implementation-owner turn count: **1**. No agents were spawned; subagent/agent wait count and wait ceiling were **0**. Test synchronization used only asynchronous-continuation TCS/event gates with bounded `WaitAsync`; no proof used `Task.Delay`, sleep, `Task.Yield`, live database/geodata, filesystem retry, or host-local time.
- Prerequisite command: `dotnet test --project tests/ImmichReverseGeo.Tests/ImmichReverseGeo.Tests.csproj --filter "FullyQualifiedName~ProcessingRunExecutorChange11Tests|FullyQualifiedName~ProcessingBackgroundServiceDelegationTests|FullyQualifiedName~ProcessingServiceRegistrationTests" --no-restore` — **31/31 passed**, 0 failed/skipped, MTP 10.842s. This proved block-11 executor/host APIs and singleton/hosted registrations before source edits.
- Coherent source build: `dotnet build --no-restore` passed; final build passed with **0 warnings, 0 errors** in 2.43s.
- Final Change-12-only focused command: `dotnet test --project tests/ImmichReverseGeo.Tests/ImmichReverseGeo.Tests.csproj --no-build --filter "FullyQualifiedName~ProcessingScheduleChange12Tests"` — **35/35 passed**, 0 failed/skipped, MTP 0.999s.
- Full focused command: `dotnet test --project tests/ImmichReverseGeo.Tests/ImmichReverseGeo.Tests.csproj --no-build --filter "FullyQualifiedName~ProcessingScheduleChange12Tests|FullyQualifiedName~ProcessingBackgroundServiceTests|FullyQualifiedName~ProcessingBackgroundServiceDelegationTests|FullyQualifiedName~ProcessingBackgroundServiceRoutingCoverageTests|FullyQualifiedName~ProcessingServiceRegistrationTests|FullyQualifiedName~ProcessingRunExecutorTests|FullyQualifiedName~ProcessingRunExecutorChange11Tests|FullyQualifiedName~ConfigServiceTests|FullyQualifiedName~ScheduleEditorStateTests|FullyQualifiedName~ProcessingStateEventReporterTests|FullyQualifiedName~ProcessingStateTests"` — **139/139 passed**, 0 failed/skipped, MTP 3.506s.
- Canonical command: `npm run test` with default Integration/Performance exclusions — **368/368 passed**, 0 failed/skipped, MTP 28.886s. Integration tests were not run because no integration-covered database, geodata, query, persistence, protocol, or external-source path changed.
- Verification: `openspec validate 12-separate-schedule-calculation-from-execution --strict` passed; `openspec status --change 12-separate-schedule-calculation-from-execution --json` reports all artifacts complete; apply reports `all_done`, **29/29 complete**, 0 remaining. `git diff --check` passed. Dependency grep found no repository/count/skipped/resolver/geodata/executor/coordinator dependency in `ProcessingSchedule.cs`; the Change-12 test has no `Task.Delay` or `Task.Yield`.
- Focused invocations: **6** including prerequisite, three investigated pre-final attempts, final Change-12-only, and full focused. The investigated failures were all caught before canonical verification: missing framework imports at first compile; an incorrect reflection assertion about record init accessors; a fake executor that initially did not complete the real reporter lifecycle; and a cancellation-registration ordering race fixed with a stable token-armed gate. No production behavioral defect or waived assertion remained.
- Catchability: the pre-edit matrix directly required buildability, immutable plan shapes, exact old reporter lifecycle, accepted-after-terminal behavior, and stable cancellation gates, so every intermediate issue was catchable and was caught by the planned coherent-build/focused gates before task completion.
- Scope review: production changes are limited to the new immutable UTC schedule plan/calculator/temporary trigger contract, the hosted schedule loop's TimeProvider-based calculation/waits and direct-host boundary implementation, and the same-singleton boundary alias registration. No `ProcessingRunExecutor`, block 1–11 data-plane contract, block-13 coordinator, Dashboard/Settings/ScheduleEditorState/config format/UI field, database/query, eligibility/count/geodata, persistence, process/protocol, or later-change artifact was modified. The temporary direct-host implementation is documented as the block-13 replacement point with all required semantics.
- Preserved baseline: the deleted active Change-02 files and untracked archived Change-02, `update-sqlite-dependencies`, lifecycle characterization spec, `.agents/`, and `.brooks-lint-history.json` remain untouched. Nothing is staged or committed; no push, sync, archive, cleanup, or later change was started.
- Blockers: **none**. Residual risk: the trigger boundary is intentionally temporary and directly implemented by the host until block 13; its replacement obligations are source-documented and executable-contract tested.

## 2026-08-30 — Change 12: implementation-owner turn 2 combined pre-audit remediation

### Independent audit findings and reopening decision

- Before any turn-2 production or test edit, reopened tasks **1.2, 1.3, 2.3, 3.1, 3.3, 3.4, 4.1, 4.3, 4.4, 4.5, 5.1, 5.3–5.8, and 6.1–6.5**. Their turn-1 completion evidence is unsupported until this coherent remediation and every final gate pass.
- Production audit finding: `ProcessingBackgroundService.ExecuteAsync` catches any `OperationCanceledException` whenever the hosted stopping token is concurrently cancelled. That violates the established active-versus-foreign taxonomy: a foreign/default-token cancellation-like failure from configuration, trigger control, executor, or reporter can be suppressed by a shutdown race. Cooperative suppression must require attribution to the exact stopping token; foreign cancellation must propagate by exact reference without recursive reporting or another trigger.
- Test/evidence audit finding: turn 1 used test-to-test wrapper methods, a meta test that checked only a short subset, broad reflection/name claims, and aggregate counts as substitutes for direct scenario proof. The local-zone assertion never drove materially different `TimeProvider.LocalTimeZone` values through the actual loop. Config tests did not retain exact Enabled/Cron snapshot histories. Startup/admission recordings were incomplete. Zero-work used a synthetic executor rather than the real block-11 executor. Preset/custom persistence did not round-trip through the actual config representation/runtime. Several retained awaits remained unbounded.
- Remediation decision: fix the cancellation classifier first; replace wrapper/meta illusions with direct independently asserted tests and a literal 29-task method map; add exact thread-safe causal recordings, fake clocks with UTC versus DST local zones, snapshot pinning histories, real-executor Scheduled zero work, exact architecture surfaces/substitute boundary proof, preset/custom round trips, and bounded gates throughout retained Change-12 scope; then run one coherent focused matrix and one canonical suite.
- Catchability: both findings were catchable from the original preflight, which explicitly claimed active-versus-foreign cancellation, one row per independently testable scenario, exact local-zone independence, exact snapshot/causal sequences, real authoritative zero eligibility, forbidden dependency surfaces, and bounded gates. Turn-1 acceptance was premature; counts are not accepted as proof in turn 2.

### Turn-2 direct requirement/scenario proof matrix

| Independently testable scenario/task | Direct executable proof | Exact assertion (not aggregate-count substitution) |
|---|---|---|
| deterministic valid expression / 2.1–2.2, 5.1 | `Calculate_FixedUtcExpressions_UseStandardStrictlyFutureUtcSemantics` | fixed hourly/daily/weekly/custom UTC occurrence and zero offset |
| current matching instant excluded / 2.2 | `Calculate_HourlyDailyWeeklyCustomAndInvalidMatrix_IsUtcAndHostZoneIndependent` | current match advances to the next matching instant |
| host local zone differs / 5.1 | `ScheduleLoop_IdenticalUtcAcrossUtcAndDstLocalZonesProducesIdenticalExactDue` | actual loop receives UTC and material custom DST zones yet produces identical 13:00Z due, 26m timer, zero offset, and two UTC reads |
| nonzero calculator input / 5.1 | `Calculate_NonZeroOffsetInput_IsRejectedAndUtcInputIgnoresLocalDst` | explicit nonzero offset rejected; UTC calendar retained |
| invalid expression / 2.2, 5.2 | `ExecuteAsync_DisabledAndInvalidRetryWaitExactDurationsBeforeFreshRead` | exact five-minute timer, no trigger/next line, reread only at advancement |
| no future occurrence / 2.2 | `Calculate_ExpressionWithoutNextOccurrence_ReturnsFiveMinuteInvalidRetry` | exact InvalidRetry five minutes |
| disabled retry / 5.2 | `ExecuteAsync_DisabledAndInvalidRetryWaitExactDurationsBeforeFreshRead` | exact one-minute timer and no trigger/next line |
| active retry shutdown / 4.4 | `ExecuteAsync_ExactStoppingTokenCancellationIsSuppressedAtConfigBoundary` and `ExecuteAsync_ShutdownDuringDisabledOrInvalidRetryProducesNoTriggerOrError` | exact stopping-token cancellation completes quietly; no ordinary error/event/trigger |
| foreign config cancellation during shutdown / defect | `ExecuteAsync_ForeignConfigCancellationPropagatesExactReferenceWhenShutdownRaces` | gated default-token OCE propagates same reference with zero trigger/event/error recursion |
| foreign downstream cancellation during shutdown / defect | `ExecuteAsync_ForeignDownstreamCancellationPropagatesExactReferenceDuringShutdownWithoutRecursiveTrigger` | same foreign OCE reference escapes after one Scheduled request; no second trigger/terminal recursion |
| future occurrence / 5.3 | `ScheduleLoop_FutureAlreadyDueAndCancelledPlansHaveCompleteExactArrays` | exact log/config/timer/trigger arrays, one trigger, no duplicate/post-terminal trigger |
| clock passed due / 5.3 | `ScheduleLoop_FutureAlreadyDueAndCancelledPlansHaveCompleteExactArrays` | no log/due timer, exact one trigger, then disabled reread; exact UTC-read count |
| shutdown before due / 4.4, 5.3 | `ScheduleLoop_FutureAlreadyDueAndCancelledPlansHaveCompleteExactArrays` | exact one planned line, zero trigger before/after later advancement, one snapshot, two UTC reads |
| positive next-run visibility / 4.2 | `ExecuteAsync_PositiveWaitPublishesOneExactUtcNextRunLineBeforeDelay` | exact one `Next run scheduled at 2026-08-30 13:00:00Z` before timer |
| disabled/invalid/already-due visibility / 4.2 | direct disabled/invalid and exact due-array proofs above | zero next-run line in each nonpositive/no-due case |
| disabled mutation / 4.3, 5.5 | `ScheduleLoop_DisabledSnapshotPinsExactCronUntilOneMinuteThenUsesEnabledReplacement` | history exactly `False|disabled-cron-A → True|0 * * * *`; replacement planned only after 1m |
| invalid mutation / 4.3, 5.5 | `ScheduleLoop_InvalidCronASnapshotPinsFiveMinutesThenUsesValidCronB` | history exactly invalid-A → valid B; B planned only after 5m; zero trigger |
| due mutation / 4.3, 5.5 | `ScheduleLoop_DueCronARemainsTriggerUntilDueThenCronBIsReadAndPlanned` | cron A remains the sole due trigger; exact history/log array then cron B timer |
| accepted execution mutation / 3.3, 4.3 | `ScheduleLoop_AcceptedRunPinsCronAAndBlocksCronBUntilTerminal` | second read task remains incomplete until terminal gate; exact A→B history |
| preset persistence/runtime / 5.5 | `PresetCron_RoundTripsConfigAndIsConsumedByRuntimeWithoutNewScheduleFields` | generated Friday preset round-trips ConfigService and loop computes exact Friday UTC due; two schedule fields |
| arbitrary custom persistence/runtime / 5.5 | `CustomCron_RoundTripsConfigAndIsConsumedByRuntimeWithoutEditorRewrite` | exact custom text round-trips and drives exact UTC due while existing editor fallback remains unchanged |
| startup lifecycle / 1.2, 4.1, 5.6 | `ExecuteAsync_StartupCausalSequenceIsLoggerInitializeReadyUiThenFirstConfigWithNoTrigger` | complete thread-safe sequence: init ILogger → initializer enter/release → ready ILogger → service UI → config; zero trigger |
| due rejection / 3.3, 5.4 | `ExecuteAsync_RejectionHasExactStartupNextRunContentionSequenceAndZeroScheduledExecution` | complete startup/next/contention UI array; one pre-existing Manual executor only; zero Scheduled request/event/accounting/extras |
| due acceptance / 3.1, 3.3, 5.4 | `ExecuteAsync_AcceptedScheduledCausalSequenceIsPendingRequestExecutorTerminalThenReread` | exact Scheduled request/token; pending before executor; exact Started→Eligibility(0)→Finished identity; terminal before second read; complete causal operation array |
| real zero-work run / 4.5, 5.7 | `ScheduledZeroWork_RealExecutorPublishesExactEventsAndTouchesNoPostCountDependency` | real ProcessingRunExecutor; exact Scheduled identity/session/event array/result counters; only authoritative count call; zero skipped/config/batch/geodata/persistence |
| manual Run Now/cancel / 5.8 | `ManualCompatibility_TwoRunsHaveExactIdentityCancellationAndNoCrossRunDuplicates` | two unique Manual requests; first Cancelled, second Completed; exact three-event arrays each, six total, no cross-run/duplicate/post-cancel event |
| schedule allowed dependency surface / 1.3, 3.4, 6.4 | `Architecture_ExactAllowedSurfacesExcludeDataPlaneAdmissionAndCoordinatorDependencies` | exact field/constructor/method type sets; no repository/count/skipped/resolver/geodata/state/executor/coordinator surface |
| DI identity/no cycle / 3.2, 4.6 | `Architecture_DIIdentityIsExactAndHasNoTriggerToHostConstructorBackEdge` | exact host field set; concrete/IHostedService/IScheduledRunTrigger ReferenceEquals; one hosted service; no trigger constructor back-edge |
| block-13 replacement / 3.1, 6.5 | `Block13SubstituteTrigger_ExercisesRejectedAndAcceptedAfterTerminalSemantics` | substitute boundary exercises both result values, exact token, and terminal gating through real schedule loop |
| future preflight disagreement / 4.5 | `SchedulerContract_CannotPublishPreflightOrEligibilityWhenFutureProbeDisagrees` | exact trigger contract has only token/result and loop has no count/eligibility/preflight API |
| literal task coverage / 6.1 | `TaskProofMap_LiterallyMapsAll29TasksToExistingDirectExecutableMethods` | literal exact 29 IDs; every mapped method resolves uniquely to a TestMethod; no self-reference or test-to-test wrapper |
| bounded retained host tasks / 5.8 | retained `ProcessingBackgroundServiceTests` | formerly unbounded pass, scheduled admission, and manual-admission tasks now use ten-second `WaitAsync` bounds |

- Wrapper TestMethods, synchronous `GetAwaiter().GetResult`, short-subset meta proof, synthetic zero executor proof, and wildcard/name-only architecture assertions were removed. Shared helpers contain mechanics only; independently asserted TestMethods no longer invoke other TestMethods.

### Turn-2 completion evidence

- Implementation-owner turn: **2**. Completion UTC: **2026-08-30T23:10:27Z**. No agents were spawned; agent/subagent wait count and wait ceiling were **0**. No user/approval wait occurred.
- Production remediation: hosted scheduling is isolated in `ProcessingScheduleLoop` with the exact allowed dependencies (schedule configuration, TimeProvider, visibility action, trigger boundary). `ProcessingBackgroundService` suppresses shutdown OCE only when `ex.CancellationToken == stoppingToken` and that exact token is cancelled. Stable gated tests prove exact-token cooperative completion and same-reference foreign config/downstream propagation during a shutdown race.
- Final build command: `dotnet build --no-restore` — passed with **0 warnings, 0 errors** (1.62s in the coherent focused invocation; earlier final-audit build also passed).
- Final coherent focused command: `dotnet test --project tests/ImmichReverseGeo.Tests/ImmichReverseGeo.Tests.csproj --no-build --filter "FullyQualifiedName~ProcessingScheduleChange12|FullyQualifiedName~ProcessingBackgroundServiceTests|FullyQualifiedName~ProcessingBackgroundServiceDelegationTests|FullyQualifiedName~ProcessingBackgroundServiceRoutingCoverageTests|FullyQualifiedName~ProcessingServiceRegistrationTests|FullyQualifiedName~ProcessingRunExecutorTests|FullyQualifiedName~ProcessingRunExecutorChange11Tests|FullyQualifiedName~ConfigServiceTests|FullyQualifiedName~ScheduleEditorStateTests|FullyQualifiedName~ProcessingStateEventReporterTests|FullyQualifiedName~ProcessingStateTests"` — **145/145 passed**, 0 failed/skipped, MTP 3.912s.
- Canonical post-coherence command (run once): `npm run test` — **374/374 passed**, 0 failed/skipped, MTP 25.519s with default Integration/Performance exclusions. No integration-covered database/geodata/query/persistence/external path changed, so integration tests were not run.
- Focused invocations in turn 2: **5**. The first 19-test audit run found two test-expectation defects (a rejection test allowed immediate same-cron replanning to add a legitimate extra line; arbitrary custom cron was incorrectly expected to round-trip through the editor fallback). Both were corrected without weakening runtime persistence assertions. Subsequent audit runs passed 19/19 then 20/20; coherent focused runs passed 144/144 then final 145/145 after the exact manual cross-run proof. No production failure remained after the cancellation fix.
- Strict/status: `openspec validate 12-separate-schedule-calculation-from-execution --strict` passed; apply is `all_done`, **29/29 complete**, 0 remaining; status reports every artifact done. `git diff --check` passed; `git diff --cached --exit-code` passed (nothing staged).
- Boundedness/static gates: retained Change-12 files contain no `GetAwaiter().GetResult`, `Task.Delay`, `Task.Yield`, or sleeps; the specifically audited ProcessingBackgroundService test tasks now use ten-second `WaitAsync`. All new gates use TCS with `RunContinuationsAsynchronously`, thread-safe recordings, and bounded waits. Config persistence tests use isolated unique directories with deterministic one-pass cleanup and no retry.
- Exact scope: no ProcessingRunExecutor production behavior, blocks 1–11 contract, block-13 coordinator, Dashboard/Settings UI, ScheduleConfig fields, ScheduleEditorState implementation, query/database/geodata/persistence/protocol/process, or later-change artifact was modified. The only retained preexisting test edit adds bounds to four host lifecycle awaits. The direct-host trigger remains temporary and the substitute-boundary test proves block-13 replacement semantics without implementing a coordinator.
- Catchability: both audit findings and the two intermediate test-expectation errors were caught by the original claimed boundaries or the new direct scenario matrix. Turn-2 rechecking occurred only after the literal 29-task method map, direct scenario proofs, final coherent focused suite, single canonical suite, strict validation/status, diff check, boundedness scan, and scope review all passed.
- Preserved baseline: deleted active Change-02 files and untracked archived Change-02, `update-sqlite-dependencies`, lifecycle characterization spec, `.agents/`, and `.brooks-lint-history.json` remain untouched. Nothing was staged, committed, pushed, synced, archived, cleaned, or started for a later change. Blockers: **none**.

## 2026-08-30 — Change 12: implementation-owner turn 3 test-contract re-review

- Production pre-audit: **APPROVED**; no production edit is authorized unless direct remediation tests expose a defect.
- Test-contract pre-audit rejected only exact mapping/assertion evidence. Before any turn-3 test/evidence edit, reopened tasks **1.2, 2.2, 3.1, 3.4, 4.4, 5.1, 5.6, 5.8, and 6.1–6.3, 6.5**.
- Re-review finding: the literal task map represented several compound clauses with one representative method, treated external command gates 6.1–6.3 as TestMethods, and did not prove the executable/external maps exactly partition all 29 task IDs. The accepted causal test did not freeze and reassert complete UI/event/config/executor/identity state across cooperative stop. The block-13 substitute test did not explicitly assert the accepted result value.
- Turn-3 decision: retain production unchanged; expand each compound executable task to every direct method required by its clauses; split external gates into a typed literal command/result map excluded from reflection; assert an exact no-overlap/no-gap 29-task partition; strengthen accepted-path before/after-stop arrays and substitute accepted-result assertions; update matrix/evidence; then run dedicated, full focused, one canonical, strict/status/diff/scope gates before rechecking tasks.

### Turn-3 corrected compound-task and external-gate matrix

| Task | Complete direct-method evidence and assertion clauses |
|---|---|
| 1.2 | startup causal sequence; exact disabled/invalid waits; Standard UTC/current-exclusion matrix; positive next line/one trigger; exact rejection; accepted terminal-before-reread; manual two-run surface/CTS; exact singleton aliases |
| 2.2 | fixed valid Standard expressions; current-match exclusion and invalid six-field expression; invalid exact five-minute retry; valid no-next exact InvalidRetry |
| 3.1 | exact minimal contract surface; substitute rejection and accepted-after-terminal with explicit Scheduled origin/token/result; production accepted Scheduled identity |
| 3.4 | exact absence of coordinator/public metadata/control API; concrete Dashboard-compatible host identity; existing host manual cancellation ownership; no preflight API |
| 4.4 | disabled and invalid retry cancellation; due-wait cancellation and no post-cancel trigger; accepted execution token propagation; exact active cancellation; foreign config/downstream same-reference propagation; manual CTS scope |
| 5.1 | valid expression UTC matrix; current exclusion; invalid expression; no-next expression; real loop UTC-versus-DST LocalTimeZone independence; nonzero-offset rejection |
| 5.6 | exact startup lifecycle plus direct actual `BackgroundService.StartAsync/StopAsync` cooperative due-wait shutdown and post-stop immutability |
| 5.8 | exact two-run Manual identity/CTS/outcome/event arrays; retained duplicate-manual exclusion; retained exact armed Manual request/reporter/token delegation |
| 6.5 | substitute explicit rejection/acceptance values, Scheduled origin/token/terminal/no extras; coordinator API absent; concrete Dashboard identity retained; existing manual ownership retained |

- The typed executable map covers **26** task IDs and associates every method with a nonempty literal semantic assertion-clause record. Reflection is applied only to this map and requires exactly one direct `[TestMethod]` per referenced name.
- The separate typed external map covers only **6.1, 6.2, 6.3**: focused MTP command/result; canonical `npm run test` command/result; strict validation plus status/apply 29/29 command/results. Tests assert external keys exactly those three, executable/external overlap is zero, union equals the literal exact 29-ID array, and combined count is 29.
- Accepted-path strengthening: before stop, the test freezes exact five-line UI, three-event identity array, four-operation config/executor array, two-snapshot history, one executor/request and terminal result. After bounded cooperative stop and two-hour clock advance, it reasserts every frozen array/count/reference unchanged, proving no post-stop operation.
- Substitute strengthening: both calls explicitly assert `ProcessingRunTrigger.Scheduled`, exact hosted token and planned result; accepted explicitly equals `AcceptedAfterTerminal`; config reread remains gated until terminal release; each branch remains exactly one trigger before and after cancellation.

### Turn-3 command evidence before task recheck

- Build plus dedicated command: `dotnet build --no-restore && dotnet test --project tests/ImmichReverseGeo.Tests/ImmichReverseGeo.Tests.csproj --no-build --filter "FullyQualifiedName~ProcessingScheduleChange12AuditTests"` — build **0 warnings/errors** in 3.05s; dedicated **22/22 passed**, 0 failed/skipped, MTP 1.195s.
- Full focused command: `dotnet test --project tests/ImmichReverseGeo.Tests/ImmichReverseGeo.Tests.csproj --no-build --filter "FullyQualifiedName~ProcessingScheduleChange12|FullyQualifiedName~ProcessingBackgroundServiceTests|FullyQualifiedName~ProcessingBackgroundServiceDelegationTests|FullyQualifiedName~ProcessingBackgroundServiceRoutingCoverageTests|FullyQualifiedName~ProcessingServiceRegistrationTests|FullyQualifiedName~ProcessingRunExecutorTests|FullyQualifiedName~ProcessingRunExecutorChange11Tests|FullyQualifiedName~ConfigServiceTests|FullyQualifiedName~ScheduleEditorStateTests|FullyQualifiedName~ProcessingStateEventReporterTests|FullyQualifiedName~ProcessingStateTests"` — **146/146 passed**, 0 failed/skipped, MTP 3.559s.
- Canonical command, run once after the coherent test batch: `npm run test` — **375/375 passed**, 0 failed/skipped, MTP 24.859s with default Integration/Performance exclusions.
- Strict validation passed. The intentional pre-recheck apply snapshot was **17/29**, matching the twelve reopened tasks; `git diff --check`, empty staging, prohibited-wait scan, status, and scope review passed at 2026-08-30T23:19:44Z.
- Production diff decision: no turn-3 production file changed; all direct tests passed against the production-pre-audit-approved implementation. No blocker or exposed production defect.
- Final recheck: strict validation passed again; apply is `all_done` with **29/29 complete**, 0 remaining; status `isComplete: true` with every artifact done; diff check and empty staging passed. Completion UTC: **2026-08-30T23:20:29Z**. Implementation-owner turn: **3**. Agents/subagents and agent wait ceilings: **0**. Nothing committed, pushed, synced, archived, staged, cleaned, or started for a later change; unrelated baseline remains untouched.

## 2026-08-30 — Change 12: implementation-owner turn 4 final test exactness review

- Before any turn-4 test/evidence edit, reopened tasks **5.3 and 6.1–6.3**.
- Final test pre-audit rejection has exactly two gaps: external evidence records use a focused-command placeholder and prose-only result shortcuts instead of literal successful turn-3 commands/numerical results; the mapped 5.3 combined proof records tokens but does not directly assert future/passed Scheduled origin and exact result values.
- Turn-4 decision: make external command/result arrays literal and equality-validated with placeholder rejection; add direct future/passed origin/result assertions; retain approved production unchanged; then run dedicated, full focused, one canonical, strict/status/diff gates before rechecking.

### Turn-4 exactness remediation and proof

- External evidence is now a typed literal array with no placeholders: task 6.1 stores the complete turn-3 focused `dotnet test --project ... --filter "FullyQualifiedName~ProcessingScheduleChange12|...|FullyQualifiedName~ProcessingStateTests"` command and exact `146/146 passed; 0 failed; 0 skipped; duration 3s 559ms` result; task 6.2 stores exact `npm run test` plus `375/375 passed; 0 failed; 0 skipped; duration 24s 859ms; default Integration/Performance exclusions`; task 6.3 stores separate exact strict, status, and apply commands with their successful valid/isComplete/all_done 29/29 result records.
- Validation contains a separate expected typed dictionary and requires record-for-record `CollectionAssert.AreEqual` for each task. It explicitly rejects either `<` or `>` in command text; nonblank-only validation was removed.
- Task 5.3's direct combined proof now asserts the future call is exactly `ProcessingRunTrigger.Scheduled` plus `RejectedAlreadyRunning`, and the passed-due call is exactly `ProcessingRunTrigger.Scheduled` plus `AcceptedAfterTerminal`, in addition to exact tokens, arrays, counts, terminal gate, zero duplicates, and zero post-cancel calls.
- Build plus dedicated command: `dotnet build --no-restore && dotnet test --project tests/ImmichReverseGeo.Tests/ImmichReverseGeo.Tests.csproj --no-build --filter "FullyQualifiedName~ProcessingScheduleChange12AuditTests"` — build **0 warnings/errors** in 5.79s; dedicated **22/22 passed**, 0 failed/skipped, MTP 0.985s.
- Full focused command: exact task-6.1 filter command — **146/146 passed**, 0 failed/skipped, MTP 3.591s.
- Canonical command, once after the coherent test batch: `npm run test` — **375/375 passed**, 0 failed/skipped, MTP 24.660s with default Integration/Performance exclusions.
- Strict validation passed. The intentional pre-recheck apply snapshot is **25/29**, exactly the four reopened tasks. `git diff --check`, empty staging, scope/status, and preserved baseline checks passed at 2026-08-30T23:25:35Z.
- Production files were unchanged in turn 4; tests exposed no production defect. Blockers: **none**.
- Final recheck: strict validation passed again; apply is `all_done`, **29/29 complete**, 0 remaining; status is complete with every artifact done; diff and empty-staging checks passed. Completion UTC: **2026-08-30T23:26:08Z**. Implementation-owner turn: **4**. Agents/subagents and agent wait ceilings: **0**. No production edits, commits, pushes, sync, archive, staging, cleanup, or later-change work; unrelated baseline remains untouched.

## 2026-08-30 — Change 12: implementation-owner turn 5 formal Brooks pass 1

- Formal Brooks pass 1 Health score: **95 — NOT APPROVED** for one Dependency Disorder.
- Before any turn-5 production/test/evidence edit, reopened tasks **1.3, 4.2, 5.7, and 6.4**.
- Finding: `ProcessingScheduleLoop` consumes block-11 `IProcessingRunConfiguration` and therefore receives full mutable `AppConfig`, contradicting the Change-12 schedule-only dependency boundary even though it reads only `Schedule` fields.
- Remediation decision: introduce immutable `ProcessingScheduleSnapshot(Enabled, Cron)` plus `IProcessingScheduleConfiguration.GetSnapshotAsync`, adapt directly on the exact singleton `ConfigService` with no duplicate owner, change the loop to the narrow boundary only, preserve one-read/reevaluation behavior, update all fakes/history/DI and exact architecture assertions, then run dedicated boundary/config, full focused, one canonical, strict/status/diff/scope gates before rechecking.

### Formal pass-1 Dependency Disorder remediation evidence

- Production: added immutable `ProcessingScheduleSnapshot(bool Enabled, string Cron)` and `IProcessingScheduleConfiguration.GetSnapshotAsync()`. `ConfigService` explicitly implements the schedule interface by reading its existing persisted `AppConfig` once and projecting only the immutable schedule values; DI aliases both run and schedule configuration interfaces to the exact same `ConfigService` singleton. There is no second configuration owner or stateful adapter.
- `ProcessingScheduleLoop` constructor/field/method surfaces now use only `IProcessingScheduleConfiguration`, `ProcessingScheduleSnapshot`, `TimeProvider`, visibility action, trigger boundary, cancellation, and plan types. Source grep reports **0** matches for `IProcessingRunConfiguration`, `AppConfig`, repositories/count/skipped/resolver/geodata, `ProcessingState`, executor, or coordinator in `ProcessingSchedule.cs`.
- Behavior: each iteration awaits exactly one `GetSnapshotAsync`, then preserves the same plan/log/wait/trigger and reevaluation points. Existing ConfigService remains the block-11 full config owner; block-11 `IProcessingRunConfiguration` and `ProcessingRunExecutor` were not revised.
- Tests/fakes: all host and schedule fakes now implement the narrow schedule interface and return immutable snapshots. Exact history queues store `ProcessingScheduleSnapshot` records, retaining disabled/invalid/due/accepted pinning assertions. Architecture proof requires the exact allowed loop/snapshot/interface field and method sets and explicitly rejects `IProcessingRunConfiguration`, `AppConfig`, every data-plane type, state, executor, and coordinator. DI proof asserts schedule configuration, run configuration, and concrete configuration all ReferenceEquals the exact ConfigService singleton.
- Typed map: tasks 1.3, 4.2, 5.7, and 6.4 now name the exact architecture, ConfigService immutable snapshot, positive-loop, real zero-work, and DI identity tests with boundary-specific semantic clauses.
- First coherent build exposed only expected test-fake migration compile errors after the new public constructor contract; all were migrated to the narrow interface. Final build `dotnet build --no-restore` passed with **0 warnings, 0 errors** in 1.59s.
- Dedicated boundary/config command selected ConfigService boundary, exact architecture/DI, and four snapshot-pinning tests — **7/7 passed**, 0 failed/skipped, MTP 1.230s.
- Full focused command — **147/147 passed**, 0 failed/skipped, MTP 3.505s.
- Canonical command, once after coherence: `npm run test` — **376/376 passed**, 0 failed/skipped, MTP 29.018s with default Integration/Performance exclusions.
- Strict validation passed. The intentional pre-recheck apply snapshot is **25/29**, exactly the four reopened tasks. `git diff --check`, empty staging, source dependency grep, scope/status, and preserved baseline checks passed at 2026-08-30T23:42:26Z.
- Formal pass timing record: pass-1 score/rejection and decision were recorded before edits; build/test timing is itemized above; final completion timestamp is recorded after task recheck. Implementation-owner turn: **5**. Blockers: **none**.
- Final recheck: strict validation passed again; apply is `all_done`, **29/29 complete**, 0 remaining; status is complete with every artifact done; diff and empty-staging checks passed. Completion UTC: **2026-08-30T23:43:06Z**. Agents/subagents and agent wait ceilings: **0**. No commit, push, sync, archive, staging, cleanup, or later-change work; unrelated baseline remains untouched.

## 2026-08-30 — Change 12: implementation-owner turn 6 formal Brooks pass 2

- Formal Brooks pass 2 rejected stale duplicated typed external results: compiled evidence still records turn-3 **146/375** instead of final turn-5 **147/376**, and `expectedExternal` duplicates the same literals, making validation self-referential.
- Before any turn-6 test/evidence edit, reopened tasks **6.1 and 6.2**.
- Remediation decision: create one deterministic copied-output JSON artifact as the sole historical command/result authority; deserialize the typed map from it; remove compiled duplicate literals/dictionary; validate exact schema/task partition/commands/results/placeholders and typed-record equality from the artifact; update final counts once only if the added TestMethod changes them; then run dedicated, focused, one canonical, strict/status/diff before rechecking.

### Formal pass-2 evidence remediation

- Authoritative artifact: `tests/ImmichReverseGeo.Tests/change12-verification-evidence.json`. The test project copies this exact file to the deterministic `AppContext.BaseDirectory/change12-verification-evidence.json` path via `CopyToOutputDirectory=PreserveNewest`; there is no directory search, retry, fallback, or second source.
- The artifact is the sole historical literal authority for tasks 6.1–6.3. It records the exact focused command and final turn-5 **147/147**, 0 failed/skipped, 3.505s result; exact `npm run test` command and final turn-5 **376/376**, 0 failed/skipped, 29.018s result with default Integration/Performance exclusions; and separate exact strict/status/apply commands/results.
- `VerificationEvidenceArtifact_LoadsTypedExternalMapAndPartitionsAll29Tasks` deserializes the artifact directly. The typed external map is constructed from—and holds the exact command-array references of—the deserialized task records. The stale compiled command/result literals, `Gate` factory, and duplicated `expectedExternal` dictionary were removed.
- The same test validates schema version/change, exact external task IDs and command-kind cardinality, the exact 29-task no-overlap/no-gap partition, deterministic command grammar, successful numeric result consistency, exclusions, strict/status/apply semantics, nonblank fields, and no `<...>` placeholders. Compiled assertions contain no focused/canonical global test-count values, so future count changes require updating only the authoritative historical artifact.
- No TestMethod was added: the existing typed-map TestMethod was replaced/renamed, leaving stable suite cardinality. Build passed with **0 warnings, 0 errors** in 3.17s. Dedicated evidence test: **1/1 passed**, 0 failed/skipped, MTP 0.988s.
- Turn-6 verification after coherence: focused command **147/147 passed**, 0 failed/skipped, MTP 3.565s; one canonical `npm run test` **376/376 passed**, 0 failed/skipped, MTP 24.843s with default exclusions. Stable counts match the artifact, so no count-update/rerun cycle was needed.
- Strict validation passed. Intentional pre-recheck apply snapshot: **27/29**, exactly tasks 6.1 and 6.2 reopened. `git diff --check` and empty staging passed at 2026-08-30T23:53:06Z. Production files were unchanged in turn 6. Implementation-owner turn: **6**. Blockers: **none**.
- Final recheck: strict validation passed again; status is complete with every artifact done; apply is `all_done`, **29/29 complete**, 0 remaining; diff and empty-staging checks passed. Completion UTC: **2026-08-30T23:53:45Z**. Agents/subagents and wait ceilings: **0**. No production edit, commit, push, sync, archive, staging, cleanup, or later-change work; unrelated baseline remains untouched.

## 2026-08-30 — Change 12: formal Brooks approval and pre-commit performance

- Formal Brooks pass 3 re-reviewed the complete current diff and issued **BROOKS APPROVED**, Health **100/100**, with zero actionable findings at 2026-08-30T23:57:23Z. Formal Brooks passes: **3**.
- Final verified gates: authoritative artifact proof 1/1, focused matrix 147/147, canonical `npm run test` 376/376 with default Integration/Performance exclusions, strict validation, complete status, apply 29/29 `all_done`, and `git diff --check`.
- Implementation started at 2026-08-30T22:36:46Z and final implementation remediation completed at 2026-08-30T23:53:45Z: **1h 16m 59s**. Formal approval followed **3m 38s** later. Implementation-owner turns: **6**.
...
- Archive commit: `5923265c3b4f729e56b9d061552bdd143cb827ed` (`chore(openspec): archive change 12`). Archive CI run `33343442722`, job `99343098924`, succeeded in **2m 26s** and completed at 2026-08-31T00:06:18Z.
- End-to-end Change-12 wall time from implementation start through successful archive CI: **1h 29m 32s**. Implementation CI plus archive CI occupied **4m 44s**. Automatic sync/archive confirmation wait: **0** under the user's standing authorization.
- Final execution totals including the three independent Brooks verification passes: **15 focused/prerequisite/dedicated invocations** and **9 canonical full-suite invocations**. Implementation-owner total: **6 turns**; Brooks total: **3 passes**; timed-out subagent wait ceilings: **2 × 600s**.
- Remediation increased proof and suite counts from the initial reported state to final focused **147/147** and canonical **376/376**. The change is synced, archived, pushed, and archive-CI-verified; no blocker condition persisted and unrelated workspace state remains untouched.
