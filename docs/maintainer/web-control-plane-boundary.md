...
Standard owns one stateless `CountBackedProcessingWorkDetector` singleton, with `IProcessingWorkDetector` resolving to that same instance. The coordinator calls it once before creating a run identity or attempting admission. The immutable request carries the Scheduled trigger and a current scheduled-launch/full-eligibility snapshot; bounded diagnostics describe implementation kind, coverage and fallback use. Only `HasWork` controls the existing launch branch. Worker requests and public settings carry none of this metadata.
...
The temporary adapter reuses the existing lazy repository count once, with the exact preflight token and unchanged SQL. It retains no request, connection or result and performs no writes. Change 58 owns the later existence-query replacement. Exact contract and factory entries here permit only this lightweight Standard ownership; they do not relax the structural dependency guard. No watermark, incremental cursor, persistence, reconciliation identity or NAS scheduling policy is introduced.
